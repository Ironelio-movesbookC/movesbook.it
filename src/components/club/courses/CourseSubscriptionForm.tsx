...
import type { CourseSubscriptionFormOptions } from '@/lib/procedures/courseSubscriptionFormOptions';

export default function CourseSubscriptionForm() {
  const router = useRouter();
  const def = getProcedureDefinition(PROCEDURE_TYPE_CODES.COURSE_SUBSCRIPTION)!;
  const client = useMemo(() => createProcedureClient(PROCEDURE_TYPE_CODES.COURSE_SUBSCRIPTION), []);
  const defaultPaymentMethods = useClubDefaultPaymentMethods();

  const [options, setOptions] = useState<CourseSubscriptionFormOptions | null>(null);
  ...
  useEffect(() => {
    Promise.all([client.fetchFormOptions<CourseSubscriptionFormOptions>(), fetchCompanies()])
      .then(([opts, co]) => {
        setOptions(opts);
  ...
  const selectedCourse = options?.courses.find((c) => c.id === courseId);
  const selectedInstructor = options?.instructors.find((i) => i.id === instructorId);
  ...
  useEffect(() => {
    if (selectedCourse) setValue(String(selectedCourse.cost));
  }, [selectedCourse]);
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto p-6 space-y-6">
        <ProcedureFormSection title="Course Subscription Movement">
          <ProcedureFormGrid>
        ...
        <ProcedureFormSection title="Course Details">
          <ProcedureFormGrid>
            <ProcedureFormCell label="Course">
              <select className={procedureInputClass} value={courseId} onChange={(e) => setCourseId(e.target.value)}>
            ...
            <ProcedureFormCell label="Instructor">
              <select className={procedureInputClass} value={instructorId} onChange={(e) => setInstructorId(e.target.value)}>
