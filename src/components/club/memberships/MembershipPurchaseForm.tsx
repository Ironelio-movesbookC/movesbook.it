...
import type { MembershipFormOptions } from '@/lib/procedures/membershipFormOptions';

export default function MembershipPurchaseForm() {
  const router = useRouter();
  const def = getProcedureDefinition(PROCEDURE_TYPE_CODES.MEMBERSHIP)!;
  const client = useMemo(() => createProcedureClient(PROCEDURE_TYPE_CODES.MEMBERSHIP), []);

  const [options, setOptions] = useState<MembershipFormOptions | null>(null);
  ...
  useEffect(() => {
    Promise.all([client.fetchFormOptions<MembershipFormOptions>(), fetchCompanies()])
      .then(([opts, co]) => {
        setOptions(opts);
  ...
  const selectedMembership = options?.memberships.find((m) => m.id === membershipId);
  const selectedPlan = options?.plans.find((p) => p.id === planId);
  ...
  useEffect(() => {
    if (selectedMembership) setValue(String(selectedMembership.cost));
  }, [selectedMembership]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!membershipId) return setError('Please select a membership.');
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto p-6 space-y-6">
        <ProcedureFormSection title="Membership Movement">
          <ProcedureFormGrid>
        ...
        <ProcedureFormSection title="Membership Details">
          <ProcedureFormGrid>
            <ProcedureFormCell label="Membership">
              <select className={procedureInputClass} value={membershipId} onChange={(e) => setMembershipId(e.target.value)}>
            ...
            <ProcedureFormCell label="Plan">
              <select className={procedureInputClass} value={planId} onChange={(e) => setPlanId(e.target.value)}>
        ...
        <ProcedureFormSection title="Payment Method">
          <ProcedureFormGrid>
            <ProcedureFormCell label="Method">
