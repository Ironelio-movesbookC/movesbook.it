...
import { WORKOUT_GOALS } from '@/components/nutrition/NutritionMealInfoModal';

interface FavoriteNutritionMealCardProps {
  workout: any;
...
export default function FavoriteNutritionMealCard({ 
  workout, 
  onDelete, 
  onOverview, 
  onUseInPlanner,
  onUpdate
}: FavoriteNutritionMealCardProps) {
  ...
  // Get nutritionFoods count
  const nutrition_foodsCount = workoutData?.nutritionFoods?.length || 0;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/nutrition/favorites/${workout.id}`, {
        method: 'PATCH',
        headers: {
      ...
      <div className="bg-white rounded-xl border-2 border-gray-200 p-6 hover:border-blue-300 hover:shadow-lg transition">
        ...
        <div className="space-y-2 mb-4">
          <div className="flex justify-between text-sm">
            ...
            <span className="font-semibold text-gray-900">{nutrition_foodsCount}</span>
          </div>
          <div className="flex justify-between text-sm">
