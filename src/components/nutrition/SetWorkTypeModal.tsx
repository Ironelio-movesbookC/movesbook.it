...
export default function SetWorkTypeModal({ nutritionFood, onClose, onSave }: SetWorkTypeModalProps) {
  const [selectedType, setSelectedType] = useState<'NONE' | 'MAIN' | 'SECONDARY'>(
    nutritionFood.workType || 'NONE'
  );
    ...
    >
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6 m-4" onClick={(e) => e.stopPropagation()}>
        <h2 className="text-xl font-bold mb-4 text-gray-900">
          Set Work Type for NutritionFood {nutritionFood.letter}
        </h2>
        
        <div className="mb-6">
          <p className="text-sm text-gray-600 mb-4">
            Sport: <span className="font-semibold">{nutritionFood.sport}</span>
          </p>
          <div className="text-sm text-gray-600 mb-4">
            Description: {(() => {
              const cleanDescription = stripCircuitTags(nutritionFood.description);
