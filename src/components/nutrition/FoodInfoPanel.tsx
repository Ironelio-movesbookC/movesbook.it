...
import { formatNutritionFoodType, getRepsLabelCap, getRepsLabel, isDistanceBasedSport } from '@/constants/nutrition-food.constants';
...
/** Build "distances only" line (e.g. 100\\A2+50\\A1+200\\B1) from nutrition_components; second return is typed description from notes. */
function getDistancesAndTypedDescription(nutritionFood: any): { distancesLine: string; typedDescription: string } {
  const nutritionComponents = nutritionFood.nutritionComponents || [];
  const distancesLine = nutritionComponents.length > 0
    ? nutritionComponents
        .map((lap: any) => {
        ...
        .join('+')
    : '';
  const typedDescription = typeof nutritionFood.notes === 'string'
    ? stripCircuitTags(nutritionFood.notes)
    : '';
...
interface NutritionFoodInfoPanelProps {
  isOpen: boolean;
  onClose: () => void;
  nutritionFood: any;
  workout: any;
  ...
  onBulkAddNutritionComponents?: () => void;
}

export default function NutritionFoodInfoPanel({
  isOpen,
  onClose,
  nutritionFood,
  workout,
...
}: NutritionFoodInfoPanelProps) {
  const [activeTab, setActiveTab] = useState<'overview' | 'nutrition_components' | 'stats'>('overview');
  ...
  let fallbackTotalNutritionComponents = 0;
  let fallbackTotalDistance = 0;
  let fallbackTotalTime = 0;
  let fallbackTotalReps = 0;
  if ((!nutritionComponents || nutritionComponents.length === 0) && fastPlannerPayload && Array.isArray(fastPlannerPayload.rows)) {
    const rows = fastPlannerPayload.rows;
    if (fastPlannerPayload.plannerType === 'aerobic') {
      fallbackTotalNutritionComponents = rows.length;
      for (const r of rows) {
      ...
      fallbackTotalNutritionComponents = rows.reduce((sum: number, r: any) => {
  ...
  const totalNutritionComponents = nutritionComponents.length || fallbackTotalNutritionComponents;
  const completedNutritionComponents = nutritionComponents.filter((ml: any) => ml.status === 'COMPLETED').length;
  const totalDistance = nutritionComponents.length > 0
    ? nutritionComponents.reduce((sum: number, ml: any) => sum + (parseInt(ml.distance) || 0), 0)
    : fallbackTotalDistance;
  const isCircuitBased = nutritionFood.isCircuitBased === true;
  const circuitData = isCircuitBased ? extractCircuitDataFromNotes(nutritionFood.notes) : null;
  ...
  const lastCircuitLetter =
    circuitRows.length > 0
      ...
      : '';
  const totalCircuitSeries = isCircuitBased ? totalNutritionComponents : 0;
  const completedCircuitSeries = isCircuitBased ? completedNutritionComponents : 0;
  const totalCircuitRepetitions = isCircuitBased
    ? nutritionComponents.reduce((sum: number, ml: any) => {
        const valueSource = isDistanceBasedSport(nutritionFood.sport) ? ml.speed : ml.reps;
        return sum + (parseInt(valueSource) || 0);
      }, 0)
    : 0;
  
  // Parse time in format: HhMM'SS" (e.g., "1h23'45"")
  const totalTime = nutritionComponents.length === 0
    ? fallbackTotalTime
    : nutritionComponents.reduce((sum: number, ml: any) => {
  ...
  const totalReps = nutritionComponents.length > 0
    ? nutritionComponents.reduce((sum: number, ml: any) => sum + (parseInt(ml.reps) || 0), 0)
    : fallbackTotalReps;

  // Get section color
  const sectionColor = nutritionFood.section?.color || '#6366f1';
  const sectionName = nutritionFood.section?.name || 'Unknown';
      ...
      >
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white p-6">
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                {useImageIcons ? (
                  <Image 
                    src={getSportIcon(nutritionFood.sport, iconType)} 
                    alt={nutritionFood.sport} 
                    width={48}
                    height={48}
                  ...
                  <span className="text-4xl">{getSportIcon(nutritionFood.sport, iconType)}</span>
                )}
                <div>
                  <h2 className="text-2xl font-bold">
                    NutritionFood {nutritionFood.letter}
                  </h2>
                  <p className="text-indigo-200 text-sm">
                    {nutritionFood.sport?.replace(/_/g, ' ')} • {formatNutritionFoodType(nutritionFood.type)}
                  </p>
                </div>
              </div>
          ...
          <div className="flex gap-2 mt-4">
            ...
            <button
              onClick={() => {
                if (confirm('Delete this nutritionFood and all its nutritionComponents?')) {
                  onDelete?.();
                  onClose();
                }
              }}
        ...
        <div className="border-b border-gray-200 bg-gray-50">
          <div className="flex">
            ...
            <button
              onClick={() => setActiveTab('nutrition_components')}
              className={`px-6 py-3 font-medium text-sm transition-colors ${
                activeTab === 'nutrition_components'
                  ? 'text-indigo-600 border-b-2 border-indigo-600 bg-white'
                  : 'text-gray-600 hover:text-gray-900'
              }`}
            >
              Total {getRepsLabel(nutritionFood.sport)} ({totalNutritionComponents})
            </button>
            <button
            ...
            <div className="space-y-6">
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-2">
                  {nutritionFood.manualMode ? 'Extended Text' : 'Description'}
                </h3>
                <div className={`bg-gray-50 rounded-lg p-4 border border-gray-200 overflow-y-auto ${
                  nutritionFood.manualMode ? 'max-h-[500px]' : 'max-h-[300px]'
                }`}>
                  {(() => {
                    if (nutritionFood.manualMode) {
                      const rawContent = (nutritionFood.notes || nutritionFood.description) || '';
                    ...
                    const isDistanceBased = distanceBasedSports.includes(nutritionFood.sport);
                    const hasNutritionComponents = nutritionComponents && nutritionComponents.length > 0;
                    const distancesOnlyParts: string[] = [];
                    if (hasNutritionComponents && isDistanceBased) {
                      for (const ml of nutritionComponents) {
                    ...
                    if (!typedDescriptionLine && nutritionFood.description) {
                      const stripped = stripCircuitTags(nutritionFood.description);
                    ...
                    const fallbackContent = stripCircuitTags(nutritionFood.description || '');
                    if (fallbackContent && fallbackContent.includes(' | ') && fallbackContent.includes('Speed ')) {
                      ...
                      if (parsedDistancesOnly) {
                        ...
                        if (!parsedTypedDesc && typeof nutritionFood.notes === 'string') {
                          const userPart = stripCircuitTags(nutritionFood.notes);
              ...
              <div>
                ...
                <div className={`grid grid-cols-2 ${nutritionFood.sport !== 'BODY_BUILDING' ? 'md:grid-cols-4' : 'md:grid-cols-3'} gap-4`}>
                  {/* Hide Total Distance for bodybuilding */}
                  {nutritionFood.sport !== 'BODY_BUILDING' && (
                  ...
                  <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
                    ...
                    <div className="text-2xl font-bold text-purple-900">
                      {isCircuitBased ? `${completedCircuitSeries}/${totalCircuitSeries}` : `${completedNutritionComponents}/${totalNutritionComponents}`}
                    </div>
                  </div>
                  <div className="bg-orange-50 rounded-lg p-4 border border-orange-200">
                    <div className="text-orange-600 text-sm font-medium mb-1">
                      {isCircuitBased ? 'Total repetitions' : `Total ${getRepsLabelCap(nutritionFood.sport)}`}
                    </div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm font-medium text-gray-900">
                        {Math.round((completedNutritionComponents / totalNutritionComponents) * 100)}%
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-3">
                      <div
                        className="bg-gradient-to-r from-green-500 to-emerald-500 h-3 rounded-full transition-all duration-500"
                        style={{ width: `${(completedNutritionComponents / totalNutritionComponents) * 100}%` }}
                      />
                    </div>
                  </div>
                </div>
            ...
            <div className="space-y-4">
              ...
              {nutritionComponents.length > 0 ? (
                <div className="space-y-2">
                  {nutritionComponents.map((nutritionComponent: any, index: number) => (
                    <div
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex items-start gap-3 flex-1">
                          {/* Status Icon */}
                          <div className="mt-1">
                            {nutritionComponent.status === 'COMPLETED' ? (
                          ...
                          {/* NutritionComponent Details */}
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              <span className="text-sm font-bold text-gray-700">
                                #{nutritionComponent.repetitionNumber || index + 1}
                              </span>
                              ...
                              {nutritionFood.sport === 'BODY_BUILDING' ? (
                                ...
                                nutritionComponent.distance && (
                                  <span className="text-sm text-gray-600">
                                    {nutritionComponent.distance}m
                                  </span>
                                )
                              )}
                              ...
                              {nutritionComponent.pause && (
                                <span className="text-xs text-gray-500">
                                  {nutritionComponentPauseFieldLabel(nutritionComponent.restType)}: {nutritionComponent.pause}
                                </span>
                              )}
                            </div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs text-gray-600">
                              {nutritionComponent.style && (
                                <div>
                                  <span className="font-medium">Style:</span> {nutritionComponent.style}
                                </div>
                              )}
                              {nutritionComponent.pace && (
                                <div>
                                  <span className="font-medium">Pace:</span> {nutritionComponent.pace}
                                </div>
                              )}
                              {nutritionComponent.time && (
                                <div>
                                  <span className="font-medium">Time:</span> {nutritionComponent.time}
                                </div>
                              )}
                              {nutritionComponent.reps && (
                                <div>
                                  <span className="font-medium">{getRepsLabelCap(nutritionFood.sport)}:</span> {nutritionComponent.reps}
                                </div>
                              )}
                            </div>

                            {nutritionComponent.notes && (
                        ...
                        <div className="flex gap-1 ml-2">
                          ...
                          <button
                            onClick={() => {
                              if (confirm('Delete this nutritionComponent?')) {
                                onDeleteNutritionComponent?.(nutritionComponent);
                              }
                            }}
                            className="p-2 text-red-600 hover:bg-red-50 rounded transition-colors"
                            title="Delete nutritionComponent"
                          >
                            <Trash2 size={14} />
                          </button>
                        </div>
                ...
                <div className="text-center py-12 text-gray-500">
                  ...
                  <p>No nutrition_components yet</p>
                  <button
                    onClick={() => {
                      onAddNutritionComponent?.();
                      onClose();
                  ...
                  >
                    Add your first nutritionComponent
                  </button>
                </div>
              )}
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg p-6 border border-green-200">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <div className="text-2xl font-bold text-green-900">{completedNutritionComponents}</div>
                    <div className="text-sm text-green-700">Completed</div>
                  </div>
                  <div>
                    <div className="text-2xl font-bold text-orange-900">{totalNutritionComponents - completedNutritionComponents}</div>
                    <div className="text-sm text-orange-700">Pending</div>
                  </div>
                  <div>
                    <div className="text-2xl font-bold text-blue-900">{totalNutritionComponents}</div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-blue-900">
                        {totalNutritionComponents > 0 ? formatDistance(totalDistance / totalNutritionComponents) : 'N/A'}
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-purple-900">
                        {totalNutritionComponents > 0 ? formatTime(totalTime / totalNutritionComponents) : 'N/A'}
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-orange-900">
                        {totalNutritionComponents > 0
                          ? Math.round((isCircuitBased ? totalCircuitRepetitions : totalReps) / totalNutritionComponents)
                          : 'N/A'}
                      </span>
                    </div>
                  </div>
        ...
        <div className="border-t border-gray-200 bg-gray-50 px-6 py-4">
          <div className="flex items-center justify-between text-xs text-gray-500">
            <div>
              NutritionFood ID: {nutritionFood.id}
            </div>
            <button
              onClick={onClose}
