...
import FastPlannerOfNutritionFoods from './FastPlannerOfFoods';
...
interface AddEditNutritionFoodModalProps {
  ...
  editingFromNutritionComponent?: boolean; // Flag to indicate editing was triggered from nutritionComponent edit
...
export default function AddEditNutritionFoodModal({
  ...
  }, [type, sport, batterySubmenu, editingFromNutritionComponent, existingNutritionFood?.isCircuitBased, existingNutritionFood?.notes, existingNutritionFood?.nutritionComponents, setBatterySubmenu]);
  ...
  useEffect(() => {
    if (editingFromNutritionComponent && editingNutritionComponentTarget) {
      console.log('🔄 [AddEditNutritionFoodModal] Editing from nutritionComponent, forcing BATTERY/circuits mode');
      setType('BATTERY');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log('  In buildNutritionFoodData result:', nutritionFoodData.manualInputType);
      console.log('  Are they equal?:', manualInputType === nutritionFoodData.manualInputType);
      console.log('  Type of form state:', typeof manualInputType);
      console.log('  Type of data:', typeof nutritionFoodData.manualInputType);
      console.log('  Is data.manualInputType undefined?:', nutritionFoodData.manualInputType === undefined);
      
      console.log('💾 [SAVE] NutritionFood data being saved:', nutritionFoodData);
  ...
  const hideUI = propHideUI || (editingFromNutritionComponent && editingNutritionComponentTarget);
    ...
    <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 ${
      ...
      <div className={`rounded-lg w-full max-w-[75vw] h-[85vh] flex flex-col ${
        ...
        {!hideUI && (
          <div className="bg-gradient-to-r from-cyan-500 to-cyan-600 text-white px-4 py-2.5 flex items-center justify-between flex-shrink-0">
            <h2 className="text-lg font-bold">
              {mode === 'add' ? 'Add NutritionFood' : 'Edit NutritionFood'}
            </h2>
            <button
              onClick={handleClose}
        ...
        <div ref={bodyRef} className={`overflow-y-auto ${hideUI ? 'bg-transparent p-0 overflow-visible' : (type === 'BATTERY' ? 'p-2 pb-2' : 'p-4 pb-64')}`} style={{ height: type === 'STANDARD' ? 'calc(80vh - 140px)' : 'calc(80vh - 100px)' }}>
          {/* Edit NutritionFood Tab - Hide in invisible mode */}
          ...
          {type === 'BATTERY' && canUseCircuitPlanner && batterySubmenu === 'circuits' && (
            <BatteryCircuitPlanner
              ...
              startInSecondView={
                editingFromNutritionComponent === true ||
                (mode === 'edit' &&
                  !!existingNutritionFood &&
                  (existingNutritionFood.isCircuitBased === true ||
                    (typeof existingNutritionFood.notes === 'string' && existingNutritionFood.notes.includes('[CIRCUIT_DATA]'))))
