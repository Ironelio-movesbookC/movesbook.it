...
import { FAST_PLANNER_REST_PAUSE_OPTIONS, getPauseOptions, getSportConfig, REST_TYPES } from '@/constants/nutrition-food.constants';
...
type AerobicPlannerRow = {
  id: number;
  /** Per-row workout section (Tools → Sections); nutritionFood primary section uses first filled row on save. */
...
const AerobicFastPlannerOfNutritionFoods = React.forwardRef<FastPlannerHandle, AerobicFastPlannerProps>(
  function AerobicFastPlannerOfNutritionFoods(
    { sport, sectionId, workoutSections = [], workout: _workout, day: _day, mode, existingNutritionFood, onSave, onCancel, fullView },
    ref,
  ) {
    ...
    const applyNutritionSectionToAllRows = useCallback((nextSectionId: string) => {
      setRows((prev) => prev.map((r) => ({ ...r, workoutSectionId: nextSectionId })));
    }, []);

    const onNutritionSectionChange = useCallback((rowId: number, nextId: string) => {
      setRows((prev) => {
    ...
    const loadedNutritionFoodIdRef = useRef<string | null>(null);

    useEffect(() => {
      if (mode !== 'edit' || !existingNutritionFood?.id) {
        loadedNutritionFoodIdRef.current = null;
        return;
      }
      ...
      const parsed = parseFastPlannerDataFromNotes(existingNutritionFood?.notes);
    ...
    }, [mode, existingNutritionFood, setActiveFieldAndRef]);
    ...
    const buildNutritionComponentsFromRows = (filled: AerobicPlannerRow[]) => {
    ...
    const handleSaveNutritionFood = (uploadToWorkout: boolean) => {
      ...
      const notes = upsertFastPlannerDataInNotes(existingNutritionFood?.notes, payload);
    ...
    React.useImperativeHandle(ref, () => ({
      saveNutritionFood: () => handleSaveNutritionFood(false),
      saveNutritionFoodAndNutritionComponents: () => handleSaveNutritionFood(true),
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {workoutSections.map((s) => (
                        <button
                          key={s.id}
                          type="button"
                          onClick={() => applyNutritionSectionToAllRows(s.id)}
...
export default AerobicFastPlannerOfNutritionFoods;
