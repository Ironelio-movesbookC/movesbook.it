...
/** NutritionComponents are not generated for fully empty grid cells (no sector and no exercise). */
function circuitStationProducesNutritionComponent(station: Station): boolean {
...
interface CircuitPlannerProps {
  ...
  initialConfig?: {
    ...
    editingFromNutritionComponent?: boolean; // Flag to indicate editing from nutritionComponent click
    ...
    editingNutritionComponentData?: any;
  };
}
...
/** Parse pause from nutritionComponent format (e.g. "0'10\"", "0'15\"", "10", 10) to seconds */
function parsePauseFromNutritionComponent(value: unknown): number {
...
function macroLoadToNutritionComponentMacroFinal(load: unknown): string | null {
  const s = String(load ?? '').trim();
...
function generatePreviewRealData(circuitsData: Circuit[]): string {
  ...
  for (const c of circuitsData || []) {
    ...
    for (const row of rows) {
      ...
      for (const st of row) {
        if (circuitStationProducesNutritionComponent(st)) slots++;
      }
    }
  }
...
export default function CircuitPlanner({ sport, onSave, onCancel, initialConfig }: CircuitPlannerProps) {
  ...
  const [hasOpenedFromNutritionComponent, setHasOpenedFromNutritionComponent] = useState(false);
  ...
  useEffect(() => {
    if (!initialConfig?.editingFromNutritionComponent || !initialConfig?.editingNutritionComponentTarget || hasOpenedFromNutritionComponent) return;
    if (!circuits.length) return;

    const target = initialConfig.editingNutritionComponentTarget;
    let circuitIdx = -1;
    ...
    const lap = initialConfig.editingNutritionComponentData;
    ...
    setHasOpenedFromNutritionComponent(true);
  }, [circuits, hasOpenedFromNutritionComponent, initialConfig?.editingFromNutritionComponent, initialConfig?.editingNutritionComponentTarget, initialConfig?.editingNutritionComponentData]);
  ...
  const generateNutritionComponents = (overrideCircuits?: Circuit[]) => {
    // 2026-01-22 10:30 UTC - Generate nutrition_components from circuit configuration
    // 2026-01-22 12:15 UTC - Fixed to use series-specific stations
    // 2026-03-30 - One nutritionComponent per grid cell (sum of station slots).
    ...
    circuitsToUse.forEach((circuit, circuitIndex) => {
      ...
      if (executionMode === 'horizontal' && seriesMode === 'count' && nSer > 0 && nSta > 0) {
        ...
        for (let jj = nSta - 1; jj >= 0; jj--) {
          const anyHere = seriesRows.some(
            (row) => row?.[jj] && circuitStationProducesNutritionComponent(row[jj]!)
          );
          if (anyHere) {
        ...
        for (let j = 0; j < nSta; j++) {
          ...
          for (let ss = 0; ss < nSer; ss++) {
            ...
            if (st && circuitStationProducesNutritionComponent(st)) producingSeriesAtCol.push(ss);
          }
          ...
          for (let s = 0; s < nSer; s++) {
            ...
            if (!circuitStationProducesNutritionComponent(station)) continue;
      ...
      seriesRows.forEach((seriesStations, seriesIdx) => {
        ...
        const producingStationIndexes = seriesStations
          .map((st, idx) => (circuitStationProducesNutritionComponent(st) ? idx : -1))
          .filter((idx) => idx >= 0);

        seriesStations.forEach((station, stationIndex) => {
          if (!circuitStationProducesNutritionComponent(station)) return;
          /** Sparse rows: only stations with sector/exercise become nutrition_components — last *used* station gets between-series / circuits / macro pause, not grid column (fixes 1 exercise + empty slots still showing only Pause among stations). */
  ...
  const handleSave = (overrideCircuits?: Circuit[]) => {
    ...
    // 2026-01-22 10:20 UTC - Include preview description for nutritionFood
    // 2026-01-22 10:30 UTC - Generate nutrition_components from circuit configuration
    // 2026-03-12 - Ensure circuit-level pause overrides (from grid) are preserved for nutrition_components display
    const circuitsToSave = overrideCircuits ?? circuits;
    // Normalize pauses from the currently visible planner values before generating nutritionComponents,
    ...
    const nutritionComponents = generateNutritionComponents(circuitsWithPauses);
    if (!nutritionComponents || nutritionComponents.length === 0) {
      alert(
    ...
    console.log('💾 [handleSave] Generated nutritionComponents:', nutritionComponents);
    console.log('💾 [handleSave] Description to save:', description);
    
    // Use circuit-level pause overrides (from grid) for config fallback when displaying nutrition_components
    ...
    const circuitData = {
      ...
      description, // Preview to be used as nutritionFood description
      nutritionComponents, // Generated nutrition_components for the nutritionFood
      config: {
        numCircuits,
        stationsPerCircuit,
        seriesMode,
  ...
  const closeManualExerciseFlow = () => {
    ...
    setShowAllSectorsInManual(false);
    if (initialConfig?.editingFromNutritionComponent) {
      onCancel();
    }
  };
  ...
  const renderExerciseSelectionModal = () => {
    ...
    const lapData = initialConfig?.editingNutritionComponentData;
    ...
    const derivedSector = (() => {
      ...
      return initialConfig?.editingNutritionComponentData?.sector || initialConfig?.editingNutritionComponentData?.muscularSector || '';
    })();

    const isEditingFromNutritionComponent = !!initialConfig?.editingFromNutritionComponent;
    ...
    const shouldFilterToSingleSector =
      ...
      !!derivedSector &&
      (isEditingFromNutritionComponent || hasSelectedSectorWithoutExercise);
                      ...
                      return (
                        <button
                          ...
                          onClick={
                            ...
                            // When editing from nutritionComponent, prefer the nutritionComponent's data as default for this station
                            const lapReps = lapData?.reps != null ? String(lapData.reps) : '';
                            const lapPause =
                              lapData != null ? parsePauseFromNutritionComponent(lapData.pause) : pauseAmongStationsBase;
                            const lapNotes = (typeof lapData?.notes === 'string' ? lapData.notes : '') || '';

                            // If re-selecting the current exercise, keep values from this exact station or nutritionComponent
      ...
      for (const circuit of next) {
        ...
        for (const seriesStations of circuit.stationsBySeries) {
          ...
          seriesStations.forEach((station, idx) => {
            // Count mode: last station pause comes from Between series / Between Circuits bars (generateNutritionComponents).
  ...
  // When editing from nutritionComponent, show only the selection modals
  if (initialConfig?.editingFromNutritionComponent) {
    return (
      <>
  ...
  return (
    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-end gap-3 py-4 border-t">
        ...
        >
          {initialConfig?.editingFromNutritionComponent || (initialConfig?.existingCircuits && initialConfig.existingCircuits.length > 0) ? 'Save' : 'Add NutritionFood'}
        </button>
        <button
      ...
      <div className={`space-y-2 ${((initialConfig as any)?.hideUI || initialConfig?.editingFromNutritionComponent) ? 'hidden' : ''}`}>
