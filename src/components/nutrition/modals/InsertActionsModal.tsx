...
type ActionRow = {
  ...
  url: string | null;
  nutritionDay: { id: string; date: string; weekNumber: number };
};
...
export default function InsertActionsModal({
  ...
  const planType = activeSection === 'B' ? 'YEARLY_PLAN' : 'MEALS_DONE';
  ...
  const loadTemplates = useCallback(async () => {
    const res = await fetch('/api/nutrition/planned-action-templates', {
      headers: authHeaders(),
    });
  ...
  const loadActions = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/nutrition/day-planned-actions?${q}`, {
        headers: authHeaders(),
      });
      const data = await res.json();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const nutritionDayId = findDayIdInPlan(nutritionPlan, dateStr, activeSection);
    if (!nutritionDayId) {
    ...
    try {
      if (editingId) {
        const res = await fetch(`/api/nutrition/day-planned-actions/${editingId}`, {
          method: 'PATCH',
        ...
        const res = await fetch('/api/nutrition/day-planned-actions', {
          method: 'POST',
          headers: { ...authHeaders(), 'Content-Type': 'application/json' },
          body: JSON.stringify(body),
        });
  ...
  const startEdit = (row: ActionRow) => {
    ...
    setUrl(row.url || '');
    const d = new Date(row.nutritionDay.date);
    setDateStr(d.toISOString().slice(0, 10));
  };

  const deleteOne = async (id: string) => {
    if (!confirm('Delete this planned action?')) return;
    const res = await fetch(`/api/nutrition/day-planned-actions/${id}`, {
      method: 'DELETE',
      headers: authHeaders(),
    });
    if (res.ok) {
  ...
  const deleteSelected = async () => {
    ...
    const res = await fetch('/api/nutrition/day-planned-actions/bulk-delete', {
      method: 'POST',
      headers: { ...authHeaders(), 'Content-Type': 'application/json' },
  ...
  const moveToDate = async (row: ActionRow) => {
    ...
    if (!next) return;
    const nutritionDayId = findDayIdInPlan(nutritionPlan, next, activeSection);
    if (!nutritionDayId) {
      alert('No day on that date in this plan.');
      return;
    }
    const res = await fetch(`/api/nutrition/day-planned-actions/${row.id}`, {
      method: 'PATCH',
      headers: { ...authHeaders(), 'Content-Type': 'application/json' },
      body: JSON.stringify({ nutritionDayId }),
    });
    if (res.ok) {
      await loadActions();
      await onSaved();
  ...
  const timelineGroups = useMemo(() => {
    ...
    for (const a of actions) {
      const key = new Date(a.nutritionDay.date).toISOString().slice(0, 10);
                  ...
                  {actions.map((row) => (
                    <tr key={row.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-2 whitespace-nowrap">
                        {formatDay(row.nutritionDay.date)}
