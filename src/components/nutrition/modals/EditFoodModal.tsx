/**
 * EditNutritionFoodModal Component
 * Modal for editing nutritionFood details
 */
...
import { SPORTS } from '@/config/nutrition.constants';
import { nutritionFoodApi } from '@/utils/api.utils';
import type { NutritionFood, Workout, NutritionDay } from '@/types/nutrition.types';
...
interface EditNutritionFoodModalProps {
...
export default function EditNutritionFoodModal({
  nutritionFood,
  workout,
  day,
  onClose,
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await nutritionFoodApi.update(nutritionFood.id, updateData);

      if (response.success) {
        onSuccess(`NutritionFood ${nutritionFood.code} updated successfully!`);
        onSave();
        onClose();
      } else {
        onError(response.error || 'Failed to update nutritionFood');
      }
    } catch (error) {
      console.error('Error updating nutritionFood:', error);
      onError('Error updating nutritionFood');
    } finally {
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-xl font-bold text-gray-900">
              Edit NutritionFood {nutritionFood.code}
            </h2>
            <p className="text-sm text-gray-500">
              NutritionMeal #{workout.sessionNumber} • {nutritionFood.sport}
            </p>
          </div>
          <button 
            onClick={onClose}
          ...
          {isManualMode ? (
                ...
                <div className="border border-gray-300 rounded-lg overflow-hidden">
                  ...
                  <div 
                    ...
                    onPaste={(e) => {
                      ...
                      console.log('📋 Paste event in EditNutritionFoodModal - allowing formatted paste');
            ...
            <>
              ...
              <div>
                ...
                <select
                  name="sport"
                  defaultValue={nutritionFood.sport}
                  required
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  disabled={isSubmitting}
                >
              ...
              <div>
                ...
                <textarea
                  name="description"
                  defaultValue={stripCircuitTags(nutritionFood.description) || ''}
              ...
              <div>
                ...
                <input
                  type="text"
                  name="sectionName"
                  defaultValue={nutritionFood.sectionName || ''}
              ...
              <div>
                ...
                <input
                  type="text"
                  name="macroRest"
                  defaultValue={nutritionFood.macroRest || ''}
              ...
              <div className="bg-gray-50 p-4 rounded-lg space-y-2">
                <h3 className="text-sm font-medium text-gray-700">NutritionFood Info</h3>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-gray-500">Code:</span>
                    <span className="ml-2 font-medium">{nutritionFood.code}</span>
                  </div>
                  {/* Hide Total Distance for bodybuilding */}
                  {nutritionFood.sport !== 'BODY_BUILDING' && (
                    <div>
                      <span className="text-gray-500">Total Distance:</span>
                      <span className="ml-2 font-medium">{nutritionFood.totalDistance || 0}m</span>
                    </div>
                  )}
                  <div>
                    <span className="text-gray-500">Total Reps:</span>
                    <span className="ml-2 font-medium">{nutritionFood.totalReps || 0}</span>
                  </div>
                  <div>
                    <span className="text-gray-500">NutritionComponents:</span>
                    <span className="ml-2 font-medium">{nutritionFood.nutritionComponents?.length || 0}</span>
                  </div>
                </div>
              </div>
            </>
