...
import { DISTANCE_BASED_SPORTS, shouldShowDistance, getDistanceUnit, formatNutritionFoodType } from '@/constants/nutrition-food.constants';
...
interface WeekTotalsModalProps {
  ...
  showNutritionComponentsDetails?: boolean; // Toggle to show/hide nutrition_components in print
}

export default function WeekTotalsModal({ isOpen, week, weeks, isMultiWeekView = false, onClose, autoPrint = false, activeSection = 'A', showNutritionComponentsDetails = false }: WeekTotalsModalProps) {
  ...
  const [showNutritionComponents, setShowNutritionComponents] = React.useState(showNutritionComponentsDetails);
  ...
  const displayWeekNotesHtml = sanitizeNutritionHtml(displayWeek?.notes);
  ...
  const calculateDailySummaries = () => {
    if (!displayWeek || !displayWeek.days) return { rows: [], totals: { meals: 0, distance: 0, series: 0, time: 0 } };
    ...
    displayWeek.days.forEach((day: any, index: number) => {
      const workoutsCount = day.meals?.length || 0;
      ...
      day.meals?.forEach((workout: any) => {
        workout.nutritionFoods?.forEach((mf: any) => {
          ...
          if (isDistanceBased) {
            // Check if this is a manual mode aerobic nutritionFood
            if (mf.manualMode) {
              ...
              // Standard mode: process nutrition_components
            ...
            if (mf.manualMode) {
              ...
              // Exclude nutrition_components where reps=1 AND pause=0 (unless it's a macro)
              const hasMacro = mf.macroRest || mf.macroFinal;
              mf.nutritionComponents?.forEach((lap: any) => {
                const reps = parseInt(lap.reps) || 1;
                const pause = parseFloat(lap.pause) || 0;
                
                // Count this nutritionComponent if:
  ...
  const calculateWeekTotals = () => {
    ...
    displayWeek.days.forEach((day: any) => {
      day.meals.forEach((workout: any) => {
        totalWorkouts++;
        const nutritionMealId = workout.id || `workout-${totalWorkouts}`;
        ...
        (workout.nutritionFoods || []).forEach((mf: any) => {
          ...
          sportWorkouts.get(sport)!.add(nutritionMealId);

          totalNutritionFoods++;
          currentTotals.nutritionFoodCount += 1;
  ...
  const { sportTotals, totalWorkouts, totalNutritionFoods, totalDistance, totalDuration } = calculateWeekTotals();
  ...
  const calculateAggregatedTotals = React.useCallback(() => {
    ...
    weeks.forEach((week: any) => {
      ...
      week.days.forEach((day: any) => {
        ...
        day.meals.forEach((workout: any) => {
          if (!workout.nutritionFoods) return;
          ...
          workout.nutritionFoods.forEach((mf: any) => {
            ...
            const currentTotals = sportMap.get(sport)!;
            currentTotals.nutritionFoodCount++;
            ...
            if (mf.manualMode) {
              ...
              // Standard mode: process nutrition_components
            if (!mf.nutritionComponents) return;
    ...
    return {
      ...
      totalNutritionFoods: totalNutritionFoodsSum,
  ...
  const handlePrintDay = React.useCallback((options: PrintOptions) => {
    ...
    if (options.printSummary && selectedDay.meals?.length > 0) {
      ...
      selectedDay.meals?.forEach((workout: any) => {
        workout.nutritionFoods?.forEach((mf: any) => {
          const sport = mf.sport || 'UNKNOWN';
          ...
          if (isDistanceBased) {
            ...
            sportData.series += mf.nutritionComponents?.length || 0;
            mf.nutritionComponents?.forEach((lap: any) => {
                // Convert seconds to minutes for time tracking
                const timeInSeconds = parseFloat(lap.time) || 0;
                sportData.time += timeInSeconds / 60;
      ...
      sportsArray.forEach(([sport, data], index) => {
        ...
        htmlContent += `
          <tr>
            ...
            ${index === 0 ? `<td rowspan="${sportsArray.length}" style="border: 1px solid #999; padding: 6pt; text-align: center;">${selectedDay.meals.length}</td>` : ''}
    ...
    // NutritionMeal Info & NutritionComponents (if selected)
    if ((options.printWorkoutInfo || options.printNutritionComponents) && selectedDay.meals?.length > 0) {
      htmlContent += '<h3 style="font-size: 13pt; margin: 16pt 0 10pt 0; font-weight: bold;">📋 Meal Details</h3>';
      
      selectedDay.meals.forEach((workout: any, workoutIndex: number) => {
        ...
        htmlContent += `
          <div style="margin-bottom: 16pt; border: 1px solid #d1d5db; padding: 10pt; background: #fafafa;">
            <h4 style="font-size: 11pt; margin: 0 0 8pt 0; font-weight: bold;">${useImageIcons ? `<img src="${sportIcon}" alt="${sportName}" style="width: 16pt; height: 16pt; vertical-align: middle; margin-right: 4pt;" />` : sportIcon} NutritionMeal ${workoutIndex + 1}: ${sportName}</h4>
        `;

        // NutritionMeal Info (if selected)
        ...
        // NutritionComponents (if selected)
        if (options.printNutritionComponents && workout.nutritionFoods?.length > 0) {
          workout.nutritionFoods.forEach((nutritionFood: any, mfIndex: number) => {
            if (nutritionFood.nutritionComponents?.length > 0) {
              const mfSport = nutritionFood.sport?.replace(/_/g, ' ') || workout.sport?.replace(/_/g, ' ') || '';
              
              htmlContent += `
                <div style="margin-bottom: 10pt;">
                  <p style="margin: 0 0 4pt 0; font-size: 9pt; font-weight: bold;">NutritionFood ${mfIndex + 1}: ${mfSport}</p>
              ...
              nutritionFood.nutritionComponents.forEach((nutritionComponent: any) => {
                // Strip HTML and circuit tags from notes
                let notes = '—';
                if (nutritionComponent.notes) {
                ...
                htmlContent += `
                  <tr>
                    <td style="border: 1px solid #ccc; padding: 3pt; text-align: center;">${nutritionComponent.repetitionNumber || '—'}</td>
                    <td style="border: 1px solid #ccc; padding: 3pt; text-align: center;">${nutritionComponent.distance || '—'}</td>
                    <td style="border: 1px solid #ccc; padding: 3pt; text-align: center;">${nutritionComponent.style || '—'}</td>
                    <td style="border: 1px solid #ccc; padding: 3pt; text-align: center;">${nutritionComponent.speed || '—'}</td>
                    <td style="border: 1px solid #ccc; padding: 3pt; text-align: center;">${nutritionComponent.pace || '—'}</td>
                    <td style="border: 1px solid #ccc; padding: 3pt; text-align: center;">${nutritionComponent.time || '—'}</td>
                    <td style="border: 1px solid #ccc; padding: 3pt; text-align: center;">${nutritionComponent.pause || '—'}</td>
      ...
      >
        ...
        <div className="overflow-y-auto flex-1 p-3 print:!overflow-visible print:!h-auto print:!flex-none print:!p-0">
                ...
                <table className="border-collapse border border-gray-300 text-sm w-full">
                  ...
                  <tbody>
                    ...
                    <tr className="bg-gray-100 font-bold">
                      ...
                      <td className="border border-gray-300 px-1 py-2 text-center text-xs">{aggregatedData.totalNutritionFoods}</td>
                  ...
                  {sportTotals.length > 0 ? (
                    <>
                      ...
                      <tr className="bg-gray-100 font-bold">
                        ...
                        <td className="border border-gray-300 px-1 py-2 text-center text-xs">{totalNutritionFoods}</td>
          ...
          <div className="space-y-6 print-daily-details">
            <h2 className="text-base font-bold text-gray-900 border-b border-gray-300 pb-1">Daily Meal Details</h2>
                    ...
                    {day.meals.map((workout: any, workoutIndex: number) => (
                      <React.Fragment key={workout.id}>
                        ...
                        <div className="border border-gray-300 rounded bg-white">
                          ...
                          <div className="px-2 py-1 flex items-center justify-between bg-blue-600">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="font-semibold text-xs text-white">
                                NutritionMeal {workout.sessionNumber}: {workout.name}
                              </span>
                            </div>
                          ...
                          {/* NutritionMeal Note */}
                          {sanitizeNutritionHtml(workout.notes) && (
                            <div className="px-3 py-2 bg-blue-50 border-b border-blue-200">
                              <div className="text-xs text-gray-700 italic">
                                <strong>Note:</strong>{' '}
                                <span dangerouslySetInnerHTML={{ __html: sanitizeNutritionHtml(workout.notes) }} />
                              </div>
                            </div>
                          )}
                          ...
                          {workout.nutritionFoods && workout.nutritionFoods.length > 0 ? (
                            <div className="overflow-x-auto">
                              <table className="text-xs border-collapse w-full">
                                ...
                                <tbody>
                                  {workout.nutritionFoods.map((mf: any, mfIndex: number) => (
                                    <React.Fragment key={mf.id}>
                                      {/* Larger space between nutritionFoods (not before first) */}
                                      {mfIndex > 0 && (
                                        <tr className="h-8">
                                      ...
                                      {/* NutritionFood row with light background */}
                                      <tr className="hover:bg-blue-100 bg-blue-50">
                                        ...
                                        <td className="border border-gray-300 px-1 py-2 text-xs text-center">
                                          {formatNutritionFoodType(mf.type)}
                                        </td>
                                        <td className="border border-gray-300 px-1 py-2 text-xs">
                                          {(() => {
                                            const html = sanitizeNutritionHtml(mf.description);
                                      ...
                                      {/* Space between nutritionFood and nutrition_components - Only show if toggle is enabled */}
                                      {showNutritionComponents && mf.nutritionComponents && mf.nutritionComponents.length > 0 && (
                                        <>
                                          <tr className="h-3">
                                            <td colSpan={5} className="border-none bg-white"></td>
                                          </tr>
                                          <tr>
                                            ...
                                            <table className="text-xs bg-gray-50 w-full">
                                              ...
                                              <tbody>
                                                {mf.nutritionComponents.map((ml: any) => (
                            ...
                            <div className="p-3 text-center text-gray-500 text-sm">No nutrition_foods</div>
                          )}
                        </div>
                      </React.Fragment>
                    ))}
