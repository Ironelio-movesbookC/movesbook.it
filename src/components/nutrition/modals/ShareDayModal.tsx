...
export default function ShareDayModal({
  ...
  const nutritionFoodCount = day.meals?.reduce((count: number, workout: any) => 
    count + (workout.nutritionFoods?.length || 0), 0) || 0;

  const dayTitle = `Workout Plan - ${dayDate}`;
  const defaultMessage = `Check out my workout plan for ${dayDate}!\n\n${workoutCount} workout(s), ${nutritionFoodCount} exercise(s)\n\n${shareableLink}`;
