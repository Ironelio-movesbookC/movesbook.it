...
import { calculateSportSummaries, type SportSummary } from '@/utils/nutritionHelpers';
import { isDistanceBasedSport } from '@/constants/nutrition-food.constants';
...
import NutritionDaySummaryCells from './NutritionDaySummaryCells';
import {
  MEAL_SYMBOLS,
  MEAL_SLOT_COUNT,
  computeDayNutritionSummary,
  getMealBySlot,
  isMealEnabled,
...
interface DayRowTableProps {
  ...
  onToggleWorkout?: (nutritionMealId: string) => void;  // Added for clickable workout numbers
  onExpandOnlyThisWorkout?: (workout: any, day: any) => void; // For expanding only one workout
  onExpandDayWithAllWorkouts?: (dayId: string, meals: any[]) => void; // For row click
    ...
    >
        ...
        >
          <div className="flex items-center justify-center">
            <div
              ...
              title={hasWorkouts ? 'Meals planned' : 'No meals planned'}
            />
          </div>
        </td>
      )}

      {/* NutritionMeal Sessions - Show numbers with symbols for each workout + Day Description */}
      ...
      >
        <div className="flex items-center justify-center gap-2">
          <div className="flex items-center gap-1">
            {[1, 2, 3, 4].map((num) => {
              const workout = getMealBySlot(day, num) || sortedWorkouts[num - 1];
              const symbol = MEAL_SYMBOLS[num]?.symbol ?? '○';
              const hasData = workout && workout.nutritionFoods && workout.nutritionFoods.length > 0;
              const mealEnabled = isMealEnabled(workout);
              const colorClass = !workout ? 'text-gray-300' : !mealEnabled ? 'text-gray-400 line-through' : hasData ? 'text-black' : 'text-gray-500';
              ...
              return (
                <span 
                  ...
                  title={workout ? `${MEAL_SYMBOLS[num]?.label || 'Meal'} — click to cycle detail` : `Meal ${num} (not created)`}
                  onClick={(e) => {
          ...
          {/* Day totals summary — shown in Total of the day column */}
      ...
      {/* Total of the day — aggregate of enabled meals */}
      <NutritionDaySummaryCells
        slot={0}
        nutrients={dayNutritionSummary.dayTotal}
        enabled
      />

      {/* Meal nutrition summaries — slots 1–4: Breakfast, Lunch, Breakfast 2, Dinner */}
      {[1, 2, 3, 4].map((slot) => (
        <NutritionDaySummaryCells
          key={slot}
          slot={slot}
          nutrients={dayNutritionSummary.bySlot[slot]}
          enabled={isMealEnabled(getMealBySlot(day, slot))}
        />
      ))}
            ...
            <div>
              ...
              <div className="text-gray-900 bg-gray-50 p-2 rounded max-h-[200px] overflow-y-auto">
                {(() => {
                  ...
                  const content = (clickedNutritionFood.manualMode && clickedNutritionFood.manualPriority)
                    ? (clickedNutritionFood.notes || clickedNutritionFood.description)
                    : (clickedNutritionFood.manualMode && !clickedNutritionFood.manualPriority)
