...
import WorkoutHierarchyView from './NutritionMealHierarchyView';
...
interface DayTableViewProps {
  ...
  onExpandDayWithAllWorkouts?: (dayId: string, meals: any[]) => void;
...
export default function DayTableView({
  ...
  // Expand state: 0 = Collapsed, 1 = Workouts only (no nutritionFoods), 2 = Workouts + NutritionFoods
  ...
  useEffect(() => {
    const loadPeriods = async () => {
      try {
        ...
        const response = await fetch('/api/nutrition/periods', {
  ...
  useEffect(() => {
    const loadWeekNotes = async () => {
      if (!nutritionPlan?.weeks || nutritionPlan.weeks.length === 0) return;

      const token = localStorage.getItem('token');
      if (!token) return;

      // Load notes for all weeks
      const notesPromises = nutritionPlan.weeks.map(async (week: any) => {
        try {
          const response = await fetch(`/api/nutrition/weeks/${week.id}/notes`, {
            headers: { 'Authorization': `Bearer ${token}` }
          });
  ...
  if (!nutritionPlan || !nutritionPlan.weeks) {
    return (
      <div className="p-8 text-center text-gray-500">
        No nutrition plan available. Click "Create Plan" to start.
      </div>
    );
  }

  const sortedWeeks = [...nutritionPlan.weeks].sort((a: any, b: any) => a.weekNumber - b.weekNumber);
  ...
  const toggleWeekWorkouts = () => {
    ...
    // Cycle through 3 states: 0 (Collapsed) -> 1 (Show workout headers only) -> 2 (Show nutritionFoods) -> 0
    ...
    if (nextState === 0) {
      ...
      // Close all workouts first (remove from expandedWorkouts to hide nutritionFoods)
      allWorkoutIds.forEach(nutritionMealId => {
        if (expandedWorkoutsSet.has(nutritionMealId) && onToggleWorkout) {
      ...
      console.log('📖 State 1: Expanding days to show workout headers (no nutritionFoods)');
      
      // First, ensure all workouts are CLOSED (so nutritionFoods are hidden)
      allWorkoutIds.forEach(nutritionMealId => {
        if (expandedWorkoutsSet.has(nutritionMealId) && onToggleWorkout) {
          onToggleWorkout(nutritionMealId);
        }
      ...
      // State 2: Show nutritionFoods (days are already open, now expand workouts)
      console.log('📖📖 State 2: Expanding workouts to show nutritionFoods (no nutritionComponents)');
      
      // Days should already be open from state 1
      // Now open all workouts to show nutrition_foods
      allWorkoutIds.forEach(nutritionMealId => {
        if (!expandedWorkoutsSet.has(nutritionMealId) && onToggleWorkout) {
          onToggleWorkout(nutritionMealId);
        }
  ...
  const handleSaveWeeklyNotes = async (data: { periodId: string; notes: string }) => {
    ...
    try {
      ...
      if (token && weekId) {
        ...
        const response = await fetch(`/api/nutrition/weeks/${weekId}/notes`, {
          method: 'PATCH',
          headers: {
  ...
  const handleMoveWeek = async (targetWeekId: string) => {
    ...
    try {
      ...
      for (let i = 0; i < sortedWeeksToMove.length; i++) {
        ...
        const response = await fetch('/api/nutrition/weeks/move', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
      ...
      <div className="bg-gray-100 relative" style={{ minHeight: '100vh', paddingTop: '0' }}>
        ...
        <div className="sticky top-0 z-50 bg-gradient-to-r from-gray-50 to-white shadow-xl border-b-2 border-gray-200">
          <div className="flex items-stretch">
            {/* Week buttons moved to NutritionSectionHeader for Section A */}
            {activeSection === 'A' && (
            ...
            onClick={() => {
               ...
               <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  title={expandState === 0 ? "Show workout headers for current week" : expandState === 1 ? "Show nutritionFoods for current week" : "Collapse current week"}
                >
                  {expandState === 0 ? 'Expand current week' : expandState === 1 ? 'Expand current week (with nutritionFoods)' : 'Collapse current week'}
                </button>
              ...
              <div key={week.id} className="mb-6">
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {/* Expand All Workouts Button - expands all workouts/nutritionFoods */}
                    <button
                      onClick={toggleWeekWorkouts}
                      ...
                      title={expandState === 0 ? "Show workout headers" : expandState === 1 ? "Show nutrition_foods" : "Collapse all"}
                    >
                      <span>{expandState === 0 ? 'Expand All' : expandState === 1 ? 'Expand (+ nutritionFoods)' : 'Collapse All'}</span>
                    </button>
                    ...
                    >
                        ...
                        <thead className="sticky-table-header">
                          ...
                          <th 
                            ...
                            style={{ width: COL_WIDTHS.meals, minWidth: COL_WIDTHS.meals, backgroundColor: colors.weekHeader, color: colors.weekHeaderText }} 
                            rowSpan={2}
                          >
                            Meals<br/>planned
                          </th>
                          <th className="border border-gray-400 px-2 py-1 text-xs font-bold bg-slate-300 text-black" colSpan={3}>
                            Total of the day
                          </th>
                        ...
                        <tr style={{ backgroundColor: colors.weekHeader, color: colors.weekHeaderText }}>
                          {/* Day total sub-headers */}
                          <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-slate-200 text-black" style={{ width: '100px', minWidth: '100px' }}>Day total</th>
                          <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-slate-200 text-black" style={{ width: '100px', minWidth: '100px' }}>Cal & Pro</th>
                          <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-slate-200 text-black" style={{ width: '200px', minWidth: '200px' }}>Carb & Fats</th>
                          {/* Meal sub-headers — Cal / macros summary */}
                          <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-green-200 text-black" style={{ width: '100px', minWidth: '100px' }}>Meal</th>
                          <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-green-200 text-black" style={{ width: '100px', minWidth: '100px' }}>Cal & Pro</th>
                          <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-green-200 text-black" style={{ width: '200px', minWidth: '200px' }}>Carb & Fats</th>
                          
                          <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-orange-200 text-black" style={{ width: '100px', minWidth: '100px' }}>Meal</th>
                          <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-orange-200 text-black" style={{ width: '100px', minWidth: '100px' }}>Cal & Pro</th>
                          <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-orange-200 text-black" style={{ width: '200px', minWidth: '200px' }}>Carb & Fats</th>
                          
                          <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-blue-200 text-black" style={{ width: '100px', minWidth: '100px' }}>Meal</th>
                          <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-blue-200 text-black" style={{ width: '100px', minWidth: '100px' }}>Cal & Pro</th>
                          <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-blue-200 text-black" style={{ width: '200px', minWidth: '200px' }}>Carb & Fats</th>
                          
                          <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-pink-200 text-black" style={{ width: '107px', minWidth: '107px' }}>Meal</th>
                          <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-pink-200 text-black" style={{ width: '80px', minWidth: '80px' }}>Cal & Pro</th>
                          <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-pink-200 text-black text-left" style={{ width: '200px', minWidth: '200px' }}>Carb & Fats</th>
                          </tr>
                        </thead>
                        <tbody>
                          {sortedWeekDays.length === 0 && (
                            <tr>
                              <td colSpan={18} className="text-center py-8 text-red-600 font-bold">
                                  ...
                                  <tr className="workout-expanded-row">
                                    <td colSpan={20} className="p-0 bg-transparent">
                                      <div className="workout-details-wrapper">
                                        ...
                                        onSaveFavoriteNutritionMeal={onSaveFavoriteNutritionMeal}
                                      ...
                                      {/* Add NutritionMeal Button - Only show if less than 4 meals */}
                                      {(!day.meals || day.meals.length < 4) && (
              ...
              <label className="text-sm font-semibold text-gray-700">Options of the selected days</label>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    title={expandState === 0 ? "Show workout headers" : expandState === 1 ? "Show nutrition_foods" : "Collapse all"}
                  >
                    {expandState === 0 ? 'Expand All' : expandState === 1 ? 'Expand (with nutritionFoods)' : 'Collapse All'}
                  </button>
        ...
        <div className="relative">
          ...
          >
            ...
            <tr style={{ backgroundColor: colors.weekHeader, color: colors.weekHeaderText }}>
              ...
              <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-slate-200 text-black" style={{ width: '100px', minWidth: '100px' }}>Cal & Pro</th>
              <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-slate-200 text-black" style={{ width: '200px', minWidth: '200px' }}>Carb & Fats</th>
              {/* M1 sub-headers - Green */}
              <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-green-200 text-black" style={{ width: '100px', minWidth: '100px' }}>Meal</th>
              <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-green-200 text-black" style={{ width: '100px', minWidth: '100px' }}>Cal & Pro</th>
              <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-green-200 text-black" style={{ width: '200px', minWidth: '200px' }}>Carb & Fats</th>
              
              {/* M2 sub-headers - Orange */}
              <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-orange-200 text-black" style={{ width: '100px', minWidth: '100px' }}>Meal</th>
              <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-orange-200 text-black" style={{ width: '100px', minWidth: '100px' }}>Cal & Pro</th>
              <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-orange-200 text-black" style={{ width: '200px', minWidth: '200px' }}>Carb & Fats</th>
              ...
              {/* M4 sub-headers - Pink */}
              <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-pink-200 text-black" style={{ width: '100px', minWidth: '100px' }}>Meal</th>
              <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-pink-200 text-black" style={{ width: '100px', minWidth: '100px' }}>Cal & Pro</th>
              <th className="border border-gray-400 px-1 py-1 text-xs font-bold bg-pink-200 text-black" style={{ width: '200px', minWidth: '200px' }}>Carb & Fats</th>
            </tr>
          </thead>
          <tbody>
                  ...
                  onClick={async () => {
                    try {
                      ...
                      const updatePromises = weeksInRange.map((week: any) =>
                        fetch(`/api/nutrition/weeks/${week.id}/period`, {
                          method: 'PATCH',
                          headers: {
                            'Content-Type': 'application/json',
                            'Authorization': `Bearer ${token}`
