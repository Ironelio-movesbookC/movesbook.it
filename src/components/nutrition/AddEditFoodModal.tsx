...
// 2026-01-29 - Fast Planner of NutritionFoods component
import FastPlannerOfNutritionFoods, { type FastPlannerHandle } from './FastPlannerOfFoods';
import AerobicFastPlannerOfNutritionFoods from './AerobicFastPlannerOfFoods';

interface AddEditNutritionFoodModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (nutritionFoodData: any) => void;
  mode: 'add' | 'edit';
  workout: any;
  day: any;
  existingNutritionFood?: any;
  onSetInsertIndex?: (index: number | null) => void;
  editingFromNutritionComponent?: boolean;
  editingNutritionComponentTarget?: { circuitLetter?: string; circuitIndex?: number; localSeriesNumber?: number; stationNumber?: number } | null;
  targetNutritionComponent?: any;
  startInSecondView?: boolean;
  /** When editing, open the same form used to create the nutritionFood: 'circuits' = Circuit Planner, 'fast' = Fast Planner */
  openInBatterySubmenu?: 'circuits' | 'fast' | null;
}

export default function AddEditNutritionFoodModal({
  isOpen,
  onClose,
  onSave,
  mode,
  ...
  onSetInsertIndex,
  editingFromNutritionComponent,
  editingNutritionComponentTarget,
  targetNutritionComponent,
  startInSecondView,
  openInBatterySubmenu
}: AddEditNutritionFoodModalProps): JSX.Element | null {
  ...
  React.useEffect(() => {
    if (isOpen) {
      console.log(`🔒 AddEditNutritionFoodModal opened - mode="${mode}", isEditMode=${mode === 'edit'}`);
      console.log('📋 Modal props:', { workout: workout?.id, day: day?.id, existingNutritionFood: existingNutritionFood?.id });
    } else {
      console.log('🔒 AddEditNutritionFoodModal closed');
    }
  }, [isOpen, mode, workout?.id, day?.id, existingNutritionFood?.id]);

  // State for annotation insert position - default to last nutritionFood index
  const [annotationInsertAfter, setAnnotationInsertAfter] = React.useState<string>(() => {
    const lastIndex = (workout?.nutritionFoods?.length || 0) - 1;
  ...
  React.useEffect(() => {
    ...
    const notes = existingNutritionFood?.notes;
    const description = typeof existingNutritionFood?.description === 'string' ? existingNutritionFood.description : '';
    ...
    // Check Fast Planner FIRST - Edit must open the form used to create this nutritionFood
    const hasFastPlannerData =
      (typeof notes === 'string' && notes.includes('[FAST_PLANNER_DATA]')) ||
      existingNutritionFood?.fastPlannerData ||
      existingNutritionFood?.isFastPlannerBased ||
    ...
    // Then check Circuit - open Circuit Planner form for circuit-based nutrition_foods
    if (
      existingNutritionFood?.isCircuitBased === true ||
      (typeof notes === 'string' && notes.includes('[CIRCUIT_DATA]')) ||
      hasCircuitNutritionComponentMeta ||
      existingNutritionFood?.circuitConfig ||
      Array.isArray(existingNutritionFood?.circuits) ||
      Array.isArray(existingNutritionFood?.rows) ||
    ...
    if (existingNutritionFood?.type === 'BATTERY' && typeof notes === 'string' && notes.includes('[CIRCUIT_DATA]')) {
      setBatterySubmenu('circuits');
      return;
    }
  }, [isOpen, mode, existingNutritionFood?.isCircuitBased, existingNutritionFood?.notes, existingNutritionFood?.nutritionComponents, existingNutritionFood?.circuitConfig, existingNutritionFood?.circuits, existingNutritionFood?.rows, existingNutritionFood?.type, existingNutritionFood?.fastPlannerData, existingNutritionFood?.isFastPlannerBased, existingNutritionFood?.description, setType, setBatterySubmenu]);

  const isEditingCircuitFromNutritionComponent = editingFromNutritionComponent && !!editingNutritionComponentTarget;
  const isEditingCircuitNutritionFood =
    mode === 'edit' &&
    !!existingNutritionFood &&
    (() => {
      const notes = typeof existingNutritionFood.notes === 'string' ? existingNutritionFood.notes : '';
      const description = typeof existingNutritionFood.description === 'string' ? existingNutritionFood.description : '';
      const hasCircuitNutritionComponentMeta = (existingNutritionFood.nutritionComponents || []).some((nutritionComponent: any) => {
        if (!nutritionComponent) return false;
        if (nutritionComponent.circuitIndex != null || nutritionComponent.circuitLetter || nutritionComponent.stationNumber != null) return true;
  ...
  const canUseCircuitPlanner =
    fpCategory === 'B' || isEditingCircuitNutritionFood || isEditingCircuitFromNutritionComponent;
  const canUseFastPlanners = mode === 'edit' || showFastPlanningsForSport(sport);
  const canUseAiNutritionFoodPlan =
    (fpCategory === 'A' || fpCategory === 'B') && !isEditingCircuitNutritionFood && !isEditingCircuitFromNutritionComponent;
  const showFastPlanningsTypeButton = mode === 'edit' || showFastPlanningsForSport(sport);

  // When editing a circuit nutritionFood, treat as BATTERY immediately so we show Circuit Planner (Part 1+2)
  // instead of STANDARD form (Workout Section, DISTANCE & REPETITIONS) on first render
  const effectiveType = isEditingCircuitNutritionFood ? 'BATTERY' : type;

  // Auto-switch to 'fast' when BATTERY mode is selected for non-section B sports
  useEffect(() => {
    const hasCircuitNutritionFood =
      existingNutritionFood?.isCircuitBased === true ||
      (typeof existingNutritionFood?.notes === 'string' && existingNutritionFood.notes.includes('[CIRCUIT_DATA]')) ||
      !!existingNutritionFood?.circuitConfig ||
      Array.isArray(existingNutritionFood?.circuits) ||
      Array.isArray(existingNutritionFood?.rows) ||
      (typeof existingNutritionFood?.description === 'string' &&
        /circuit/i.test(existingNutritionFood.description) &&
        /(station|series|pause)/i.test(existingNutritionFood.description)) ||
      (existingNutritionFood?.nutritionComponents || []).some((nutritionComponent: any) => {
        if (!nutritionComponent) return false;
        if (nutritionComponent.circuitIndex != null || nutritionComponent.circuitLetter || nutritionComponent.stationNumber != null) return true;
        return typeof nutritionComponent.notes === 'string' && nutritionComponent.notes.includes('[CIRCUIT_META]');
      });
    if (
      ...
      !editingFromNutritionComponent &&
      !hasCircuitNutritionFood
    ) {
      setBatterySubmenu('fast');
    }
  }, [type, sport, batterySubmenu, editingFromNutritionComponent, existingNutritionFood?.isCircuitBased, existingNutritionFood?.notes, existingNutritionFood?.nutritionComponents, existingNutritionFood?.circuitConfig, existingNutritionFood?.circuits, existingNutritionFood?.rows, existingNutritionFood?.description, setBatterySubmenu]);
  ...
  useEffect(() => {
    if (editingFromNutritionComponent && editingNutritionComponentTarget) {
      console.log('🔄 [AddEditNutritionFoodModal] Editing from nutritionComponent, forcing BATTERY/circuits mode (nutritionComponent)');
      setType('BATTERY');
      setBatterySubmenu('circuits');
  ...
  }, [editingFromNutritionComponent, editingNutritionComponentTarget, setType, setBatterySubmenu, setManualMode]);

  // When editing a circuit nutritionFood: force BATTERY/circuits and go to Circuit Planner
  // startInSecondView is set by parent when Edit is clicked on a circuit nutritionFood
  useEffect(() => {
    if (!isOpen) return;
    if (mode !== 'edit') return;
    if (!startInSecondView && !isEditingCircuitNutritionFood) return;
    setType('BATTERY');
    setBatterySubmenu('circuits');
    setManualMode(false);
  }, [isOpen, mode, startInSecondView, isEditingCircuitNutritionFood, setType, setBatterySubmenu, setManualMode]);
  ...
  // NutritionMeal Sections state (loaded from Personal Settings - NutritionMeal Sections)
  const [workoutSections, setNutritionSections] = useState<any[]>([]);

  // Load workout sections from Prisma database
  useEffect(() => {
    const loadNutritionSections = async () => {
      try {
        const token = localStorage.getItem('token');
        if (!token) return;
        
        // Fetch workout sections from database (user's custom workout sections)
        const response = await fetch('/api/nutrition/sections', {
          headers: {
  ...
  useEffect(() => {
    if (workoutSections.length > 0) {
      console.log('📋 NutritionMeal Sections loaded:', workoutSections);
      console.log('📋 NutritionMeal Section count:', workoutSections.length);
  ...
  const handleSave = async () => {
    ...
    try {
      const nutritionFoodData = buildNutritionFoodData();
      console.log('🔹 [MODAL] Built nutritionFood data:', {
        ...
        manualInputType: nutritionFoodData.manualInputType,
        manualRepetitions: nutritionFoodData.manualRepetitions,
        manualDistance: nutritionFoodData.manualDistance
      });
      
      await onSave(nutritionFoodData);
      onClose();
      resetForm();
    } catch (error) {
      console.error('❌ [MODAL] Error saving nutritionFood:', error);
      alert('Error saving nutritionFood: ' + (error instanceof Error ? error.message : String(error)));
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const renderAnaerobicFastPlannerMacroFooter = () => (
    <div className="flex items-end gap-2 px-2.5 py-1.5 shadow-sm">
      ...
      <button
        ...
        className="mb-0.5 rounded-md bg-purple-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-purple-700"
      >
        Apply
      </button>
    </div>
  ...
  const hideNutritionFoodTopMetaInEdit =
  ...
  // For manual mode nutritionFoods, force manual tab and disable other tabs
  // Only restrict tabs if editing an EXISTING manual nutritionFood (not when creating new one)
  const isEditingManualNutritionFood = mode === 'edit' && existingNutritionFood?.manualMode;
  
  // Auto-switch to manual tab when editing a manual nutritionFood
  React.useEffect(() => {
    if (isOpen && isEditingManualNutritionFood && activeTab !== 'manual') {
      setActiveTab('manual');
    }
  }, [isOpen, isEditingManualNutritionFood, activeTab]);

  const handleTabChange = (tab: 'edit' | 'manual' | 'favorites') => {
    // Prevent tab change if editing an EXISTING manual nutritionFood (edit mode with manualMode from DB)
    // But allow tab switching when CREATING a new nutritionFood (add mode)
    if (mode === 'edit' && existingNutritionFood?.manualMode && tab !== 'manual') {
      // This is editing an existing manual nutritionFood - restrict to manual tab only
      return;
    }
  ...
  React.useEffect(() => {
    if (isOpen && type === 'ANNOTATION' && mode === 'add' && onSetInsertIndex) {
      const lastIndex = (workout?.nutritionFoods?.length || 0) - 1;
      if (lastIndex >= 0) {
        onSetInsertIndex(lastIndex + 1); // Insert after last nutritionFood
      } else {
        onSetInsertIndex(null); // No nutritionFoods, append at end
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isOpen, type, mode, workout?.nutritionFoods?.length]);
  ...
  React.useEffect(() => {
    if (type === 'STANDARD' && activeTab === 'manual' && !manualMode) {
      ...
      console.log('🔄 [MODAL] Switching to manual tab, enabling manual mode');
        setManualMode(true);
      // Only set manualPriority to true for NEW manual nutritionFoods (not when editing existing ones)
      if (mode === 'add') {
        setManualPriority(true);
      }
    }
    // NOTE: We don't automatically disable manual mode when switching to edit tab
    // because the nutritionFood might have been created in manual mode and we want to preserve that
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // This happens when editing a specific circuit nutritionComponent - we only want to show the exercise selection modal
  const hideUI = editingFromNutritionComponent && editingNutritionComponentTarget;

  if (!isOpen) return null;

  // 2026-02-02 - If editing from nutritionComponent (just for the inner modal), render a transparent container
  // This allows the inner "Edit Exercise" modal to be visible without the "Edit NutritionFood" modal frame
  if (editingFromNutritionComponent) {
    return (
              ...
              {effectiveType === 'BATTERY' ? (
                <BatteryCircuitPlanner 
                  ...
                  onCreateCircuit={async (data) => {
                    console.log('✅ [AddEditNutritionFoodModal] Circuit data received:', data);
                    console.log('✅ [AddEditNutritionFoodModal] Circuit description:', data.description);
                    console.log('✅ [AddEditNutritionFoodModal] Description length:', data.description?.length || 0);

                    const nutritionFoodData = buildNutritionFoodData();
                    const circuitMacro =
                      circuitLoadOfWorkToMacroFinal(data.config?.loadOfWork ?? data.settings?.loadOfWork);
                    const lapCount = (data.nutritionComponents || []).length;
                    const finalData = {
                      ...nutritionFoodData,
                      ...
                      rows: data.rows,
                      nutritionComponents: data.nutritionComponents || [],
                      isCircuitBased: true,
                      repetitions: lapCount > 0 ? lapCount : nutritionFoodData.repetitions,
                      macroFinal: circuitMacro ?? nutritionFoodData.macroFinal ?? "0'"
                    };

                    console.log('✅ [AddEditNutritionFoodModal] Final nutritionFood data:', finalData);
                    console.log('✅ [AddEditNutritionFoodModal] Final description:', finalData.description);
                    try {
                      await onSave(finalData);
                      onClose();
                    } catch (error) {
                      console.error('❌ [AddEditNutritionFoodModal] Circuit save failed:', error);
                      alert('Failed to save nutritionFood: ' + (error instanceof Error ? error.message : String(error)));
                    }
                  }}
                  onCancel={onClose}
                  existingNutritionFood={existingNutritionFood}
                  startInSecondView={true}
                  editingNutritionComponentTarget={editingNutritionComponentTarget}
                  targetNutritionComponent={targetNutritionComponent}
                />
              ) : (
      ...
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-[75vw] h-[92vh] flex flex-col animate-slideUp">
        ...
        <div className="bg-gradient-to-r from-cyan-500 to-cyan-600 text-white px-4 py-2.5 flex items-center justify-between flex-shrink-0">
          <h2 className="text-lg font-bold">
            {mode === 'add' ? 'Add Dietframe' : 'Edit Dietframe'}
          </h2>
        ...
        {/* Body — fast planner in modal: flex-1 + hidden overflow so FastPlannerOfNutritionFoods can scroll internally */}
        <div
          ref={bodyRef}
        ...
        >
          ...
          {errors.general && (
            <div className="mb-2 p-2 bg-red-100 border border-red-400 text-red-700 rounded text-sm">
              ...
              {/* Favorite Sports Quick Selection - hidden when editing circuit nutritionFood */}
              {!hideTopChromeForNotAerobicFast && !isEditingCircuitNutritionFood && loadingFavorites && (
              ...
              {!hideTopChromeForNotAerobicFast && !isEditingCircuitNutritionFood && !loadingFavorites && favoriteSports.length === 0 && (
              ...
              {!hideTopChromeForBatteryPlannerForms && !isEditingCircuitNutritionFood && !hideNutritionFoodTopMetaInEdit && !loadingFavorites && favoriteSports.length > 0 && (
          ...
          {/* Sport Selection - hidden when editing circuit nutritionFood (streamlined Part 1 view) */}
          {!hideTopChromeForBatteryPlannerForms && !isEditingCircuitNutritionFood && !hideNutritionFoodTopMetaInEdit && (
          <div className={`mb-3 ${mode === 'edit' ? 'relative' : ''}`}>
            ...
            <div className="flex items-center justify-center gap-3">
              ...
              >
                ...
                <optgroup label="Others - technical nutrition_foods" style={{ color: '#1d4ed8', fontWeight: 700 }}>
          ...
          {/* Type Selection */}
          {!hideTopChromeForBatteryPlannerForms && !hideNutritionFoodTopMetaInEdit && (
          <div className={`mb-3 ${mode === 'edit' ? 'relative' : ''}`}>
            {mode === 'edit' && (
              <div 
                className="absolute inset-0 z-[9999] cursor-not-allowed bg-gray-50 bg-opacity-50" 
          ...
          {effectiveType === 'BATTERY' &&
            ...
            !hideNutritionFoodTopMetaInEdit && (
            <div className="mb-3 rounded-lg border border-sky-200 bg-sky-50/70 px-3 py-2.5">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {canUseAiNutritionFoodPlan && (
                  ...
                  >
                    Plan of NutritionFoods with AI
                  </button>
                )}
              </div>
            </div>
          ...
          {/* NutritionMeal Section Selection - Only for STANDARD and BATTERY modes */}
          ...
          <div className="mb-3 w-1/2 mx-auto">
            ...
            <label className="block text-xs font-bold text-gray-700 mb-1">
                NutritionMeal Section <span className="text-red-500">*</span>
            </label>
              <div className="flex items-center gap-2">
            <select
              value={sectionId}
                  onChange={(e) => {
                    ...
                    console.log('🔄 NutritionMeal section selected:', selectedValue);
            ...
            <div className="space-y-3">
                    ...
                    onChange={(e) => {
                      ...
                      if (onSetInsertIndex) {
                        if (value === 'last') {
                          ...
                          onSetInsertIndex(index + 1); // Insert AFTER the selected nutritionFood
                        }
                      }
                    }}
                    className="w-full px-3 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 text-sm bg-white"
                  >
                    {workout?.nutritionFoods?.map((mf: any, index: number) => {
                    ...
                    <option value="last">🔽 After last nutritionFood (at the end)</option>
                  </select>
                  <p className="text-xs text-gray-500 mt-1">
                    ✨ Default: After last nutritionFood. Select a different position if needed.
                  </p>
                </div>
              )}
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Add a note or highlight for this nutritionFood..."
                />
              </div>
          ...
          {type === 'STANDARD' && manualMode && (
            <div className="p-4 bg-yellow-50 border border-yellow-300 rounded">
              <p className="text-sm text-yellow-800">
                ⚠️ Manual mode is enabled. Switch to "Edit NutritionFood" tab to see form fields, or use the "Manual Content" tab to enter distance for statistics.
              </p>
            </div>
          )}
                    ...
                    <>
                      ...
                      <div className="mb-3">
                        ...
                        <p className="mt-1 text-[10px] text-gray-500">
                          Optional: Select a technique from Tools settings to apply to this nutritionFood
                        </p>
                      </div>
          ...
          {/* 2026-01-24 - Pass existingNutritionFood for edit mode */}
          {effectiveType === 'BATTERY' && canUseCircuitPlanner && batterySubmenu === 'circuits' && (
            <BatteryCircuitPlanner
              ...
              day={day}
              existingNutritionFood={existingNutritionFood}
              startInSecondView={!!editingFromNutritionComponent || !!startInSecondView}
              editingNutritionComponentTarget={editingNutritionComponentTarget}
              targetNutritionComponent={targetNutritionComponent}
              onCreateCircuit={async (circuitData) => {
                // 2026-01-21 20:00 UTC - Handle circuit creation
                // 2026-01-22 10:20 UTC - Include description in nutritionFood data
                // 2026-01-22 10:30 UTC - Include nutrition_components in nutritionFood data
                // 2026-01-23 - Updated for redesigned planner
                console.log('✅ [AddEditNutritionFoodModal] Circuit data received:', circuitData);
                console.log('✅ [AddEditNutritionFoodModal] Circuit description:', circuitData.description);
                console.log('✅ [AddEditNutritionFoodModal] Description length:', circuitData.description?.length || 0);
                
                // Build complete nutritionFood data with circuit description and nutrition_components
                const nutritionFoodData = buildNutritionFoodData();
                ...
                const lapCount = (circuitData.nutritionComponents || []).length;
                const finalData = {
                  ...nutritionFoodData,
                  ...
                  nutritionComponents: circuitData.nutritionComponents || [], // Include generated nutrition_components
                  isCircuitBased: true, // Flag to indicate this is a circuit-based nutritionFood
                  repetitions: lapCount > 0 ? lapCount : nutritionFoodData.repetitions,
                  macroFinal: circuitMacro ?? nutritionFoodData.macroFinal ?? "0'"
                };
          ...
          {effectiveType === 'BATTERY' && isCircuitFeatureSport(sport) && batterySubmenu === 'fast' && (
            ...
            <FastPlannerOfNutritionFoods
              ...
              onSave={async (nutritionFoodData: any) => {
                try {
                  await onSave(nutritionFoodData);
                  onClose();
                } catch (error) {
                  console.error('❌ [AddEditNutritionFoodModal] Fast Planner save failed:', error);
                  alert('Failed to save nutritionFood: ' + (error instanceof Error ? error.message : String(error)));
                }
              }}
              onCancel={onClose}
          ...
          {effectiveType === 'BATTERY' && isCircuitFeatureSport(sport) && batterySubmenu === 'ai' && (
            <div className="p-8 text-center bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg">
              <h3 className="text-lg font-bold text-gray-700 mb-2">Plan of NutritionFoods with AI</h3>
          ...
          {effectiveType === 'BATTERY' && AEROBIC_SPORTS.includes(sport as any) && batterySubmenu === 'fast' && (
            ...
            <AerobicFastPlannerOfNutritionFoods
              ref={fastPlannerRef}
              sport={sport}
              sectionId={sectionId}
              workout={workout}
              ...
              onSave={async (nutritionFoodData: any) => {
                try {
                  ...
                  console.error('❌ [AddEditNutritionFoodModal] Aerobic Fast Planner save failed:', error);
                  alert('Failed to save nutritionFood: ' + (error instanceof Error ? error.message : String(error)));
                }
              }}
              onCancel={onClose}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <div className="font-semibold text-amber-900 mb-1">
                        📝 Editing Manual NutritionFood
                      </div>
                      <p className="text-sm text-amber-800">
                        This nutritionFood was created with manual input. You can edit the <strong>{DISTANCE_BASED_SPORTS.includes(sport as any) ? 'distance' : 'series'}</strong> (for statistics) and the <strong>content</strong> below. To change sport or other settings, you need to create a new nutritionFood.
                      </p>
                    </div>
                  </div>
                </div>
                    ...
                    <div className="relative">
                      {(!manualContent || manualContent === '' || manualContent === '<br>') && (
                        <div className="absolute top-2 left-3 text-sm text-gray-400 pointer-events-none z-0">
                          Here you can edit freely your nutritionFood that will be showed in the nutritionFood section
                        </div>
                      )}
                      <div
                        ref={editorRef}
        ...
        {type === 'BATTERY' && isCircuitFeatureSport(sport) && batterySubmenu === 'fast' && (
          <div className={`flex-shrink-0 border-t bg-white px-4 py-3 flex items-center ${isFastPlannerFullView ? 'justify-center' : 'justify-between'}`}>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                onClick={() => fastPlannerRef.current?.saveNutritionFoodAndNutritionComponents()}
        ...
        {type !== 'BATTERY' && (
          <div className="border-t bg-gray-50 px-6 py-4 flex items-center justify-between flex-shrink-0">
            ...
            >
              {isSaving ? 'Saving...' : mode === 'add' ? 'Add NutritionFood' : 'Save Changes'}
            </button>
          </div>
        )}
      </div>
