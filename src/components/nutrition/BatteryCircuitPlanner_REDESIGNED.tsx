...
import { CIRCUIT_SERIES_PAUSE_OPTIONS } from '@/constants/nutrition-food.constants';
...
interface BatteryCircuitPlannerProps {
  ...
  onCancel: () => void;
  existingNutritionFood?: any; // For edit mode
  ...
  targetNutritionComponent?: any; // The specific nutritionComponent being edited (for circuit mode)
  hideUI?: boolean;
}

// Helper function to extract circuit data from nutritionFood notes
...
const buildCircuitsFromNutritionComponents = (nutritionComponents: any[], fallbackConfig: any) => {
...
export default function BatteryCircuitPlanner({
  ...
  targetNutritionComponent: _targetNutritionComponent,
  hideUI: _hideUI
}: BatteryCircuitPlannerProps) {
  const existingCircuitData = React.useMemo(() => {
    if (!existingNutritionFood) return null;
    const fromNotes = extractCircuitData(existingNutritionFood.notes);
    ...
    const fallback = {
      config: existingNutritionFood.circuitConfig || existingNutritionFood.config || {},
      circuits: Array.isArray(existingNutritionFood.circuits) ? existingNutritionFood.circuits : null,
      rows: Array.isArray(existingNutritionFood.rows) ? existingNutritionFood.rows : null
    };
    if (!fallback.circuits && Array.isArray(existingNutritionFood.nutritionComponents) && existingNutritionFood.nutritionComponents.length > 0) {
      const built = buildCircuitsFromNutritionComponents(existingNutritionFood.nutritionComponents, fallback.config);
      if (built) {
        return built;
      }
    }
  ...
  const [description, setDescription] = useState(existingNutritionFood?.description || '');
  ...
  // Auto-navigate to second view if requested (e.g., when editing a nutritionComponent)
  useEffect(() => {
    ...
    return (
      <CircuitPlanner_OLD
        ...
        initialConfig={{
          ...
          editingFromNutritionComponent: !!editingNutritionComponentTarget,
          editingNutritionComponentTarget: editingNutritionComponentTarget,
          editingNutritionComponentData: _targetNutritionComponent
        }}
        onSave={(data: any) => {
        ...
        onCancel={() => {
          if (startInSecondView) {
            onCancel(); // Close the parent modal completely if we started in second view (e.g. from nutritionComponent edit)
          } else {
            setShowOldCircuitPlanner(false);
          }
        }}
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between pt-4 border-t">
        ...
        <div className="flex gap-3">
          ...
          >
            {existingNutritionFood ? 'Edit circuit' : 'Create circuit'}
          </button>
        </div>
      </div>
    </div>
