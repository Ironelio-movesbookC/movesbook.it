...
export default function AddEditMoveframeModal({
  ...
  // Auto-switch circuits → fast only when this sport has no separate Circuits planner (not category B).
  useEffect(() => {
  ...
  React.useEffect(() => {
    if (!isOpen || mode !== 'edit' || !existingMoveframe) return;
    const notes = typeof existingMoveframe.notes === 'string' ? existingMoveframe.notes : '';
    const description =
      typeof existingMoveframe.description === 'string' ? existingMoveframe.description : '';
    const hasFastPlannerData =
      notes.includes('[FAST_PLANNER_DATA]') ||
      existingMoveframe.fastPlannerData ||
      existingMoveframe.isFastPlannerBased ||
      description.toLowerCase().includes('fast planner');
    if (!hasFastPlannerData) return;
    setBatterySubmenu('fast');
  ...
  /**
   * Hide favorites, sport, type row, and workout section when a battery planner is active (add + edit)
   * so the planner gets more vertical space.
   */
        ...
        <div ref={bodyRef} className={`overflow-y-auto ${hideUI ? 'bg-transparent p-0 overflow-visible' : (type === 'BATTERY' ? 'p-2 pb-2' : 'p-4 pb-64')}`} style={{ height: type === 'STANDARD' ? 'calc(80vh - 140px)' : 'calc(80vh - 100px)' }}>
          ...
          {errors.general && (
            <div className="mb-2 p-2 bg-red-100 border border-red-400 text-red-700 rounded text-sm">
              ...
              {!hideTopChromeForBatteryPlannerForms && loadingFavorites && (
              ...
              {!hideTopChromeForBatteryPlannerForms && !loadingFavorites && favoriteSports.length === 0 && (
            ...
            >
              ...
              {showFastPlanningsTypeButton ? (
                <button
                  ...
                  disabled={mode === 'edit'}
                  className={`min-w-[8rem] flex-1 px-3 py-2 text-sm font-medium rounded border-2 transition-colors ${
                    type === 'BATTERY' && (batterySubmenu === 'fast' || !canUseCircuitPlanner)
                      ? 'bg-sky-100 border-sky-400 text-sky-950 shadow-sm'
                      : 'bg-white border-gray-300 text-gray-700 hover:bg-gray-50'
                  } ${mode === 'edit' ? 'cursor-not-allowed opacity-50 pointer-events-none' : ''}`}
                >
          ...
          {type === 'BATTERY' && showFastPlanningsTypeButton && !streamlinedBatteryFastPlanner && (
          ...
          {(type === 'STANDARD' || type === 'BATTERY') && !hideTopChromeForBatteryPlannerForms && (
          <>
