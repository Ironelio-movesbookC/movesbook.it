...
interface ImportWorkoutSharedUsersModalProps {
  isOpen: boolean;
  onClose: () => void;
  onBack: () => void;
  targetDay: { id: string };
...
export default function ImportWorkoutSharedUsersModal({
  isOpen,
  onClose,
  onBack,
  targetDay,
  targetWorkout,
  onConfirm,
}: ImportWorkoutSharedUsersModalProps) {
  ...
  const loadRecords = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch('/api/workouts/shared-weekly-plans?recordType=WORKOUT', {
  ...
  const renderCard = (record: WorkoutArchiveGridRecord) => {
    ...
    return (
      ...
      >
        ...
        {(record.sharedByUsername || record.authorFullName) && (
          <p className="text-xs text-violet-700 font-medium mb-2 truncate">
            Shared by {record.authorFullName || record.sharedByUsername}
          </p>
        )}
          ...
          <div className="p-4 sm:p-5 overflow-y-auto flex-1 space-y-4">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-3 py-2 px-4 bg-white border border-gray-200 rounded-xl flex-1 min-w-[200px]">
                <Share2 className="w-8 h-8 text-violet-600 shrink-0" />
                <span className="font-semibold text-gray-900">
                  Import from <span className="text-violet-700">shared by users</span>
                </span>
              </div>
            ...
            <p className="text-sm text-violet-900 bg-violet-50 border border-violet-200 rounded-lg p-3">
              Workouts shown here were exported to a user&apos;s General Archive and tagged as{' '}
              <strong>shareable</strong>. To share your own workout, copy it to your Archive and
              publish it with the shareable tag.
            </p>
            ...
            {showFilters && (
              <p className="text-xs text-gray-600 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
                Only workouts from other users&apos; General Archives with the{' '}
                <strong>shareable</strong> tag. Filter by sport category, search, or custom tags.
              </p>
            )}
            ...
            {loading ? (
              <p className="text-center py-10 text-gray-500 text-sm">Loading shared workouts…</p>
            ) : filtered.length === 0 ? (
              <p className="text-center py-10 text-amber-800 bg-amber-50 border border-amber-200 rounded-lg text-sm">
                No shareable workouts from other users yet. Users must export a workout to their
