...
type StructureSelection = { dayOfWeek: number; session: number };
...
interface ImportWorkoutFromStructureModalProps {
  isOpen: boolean;
  onClose: () => void;
  onBack: () => void;
  targetDay: { id: string };
...
function Sport1Cells({
  ...
  const tdBase = `border border-gray-300 p-2 align-top text-xs min-w-[72px] ${cellBg}`;

  if (!available) {
    return (
      <>
        <td className={`${tdBase} text-center text-gray-500`}>—</td>
        <td className={`${tdBase} text-center text-gray-500`}>—</td>
        <td className={`${tdBase} text-center text-gray-500`}>—</td>
      </>
    );
...
export default function ImportWorkoutFromStructureModal({
  isOpen,
  onClose,
  onBack,
  targetDay,
  targetWorkout,
  onConfirm,
}: ImportWorkoutFromStructureModalProps) {
  ...
  const selectedPlanned = useMemo(() => {
    ...
    const ids = planData.grid[selection.dayOfWeek]?.[selection.session] || [];
    const firstId = ids[0];
    return firstId ? plannedById.get(firstId) : null;
  }, [selection, planData.grid, plannedById]);

  const selectionLabel = useMemo(() => {
    if (!selection) return null;
    const dayName = DAY_NAMES[selection.dayOfWeek - 1] ?? `Day ${selection.dayOfWeek}`;
    const sport = selectedPlanned?.sportKey.replace(/_/g, ' ') ?? 'Workout';
    return `${dayName} · WO ${selection.session} · ${sport} (Plan ${planKey})`;
  }, [selection, selectedPlanned, planKey]);

  const handleSelectRow = (dayOfWeek: number, session: number, selectable: boolean) => {
    if (!selectable) return;
    setSelection((prev) =>
      prev?.dayOfWeek === dayOfWeek && prev?.session === session
        ? null
        : { dayOfWeek, session }
    );
    setConfirmOverwrite(false);
    ...
    try {
      await onConfirm({
        ...
        source: {
          ...
          planKey,
          planData,
          structureDayOfWeek: selection.dayOfWeek,
          structureSessionNumber: selection.session,
        },
      });
      onClose();
    } catch (e) {
        ...
        >
          ...
          <div className="p-4 sm:p-5 overflow-y-auto flex-1 space-y-4">
                    ...
                    {DAY_NAMES.map((dayName, dayIndex) => {
                      ...
                      return WEEKLY_STRUCTURE_SESSIONS.map((session) => {
                        ...
                        const firstPlanned = firstPlannedId
                          ...
                          : undefined;
                        const isSelected =
                          selection?.dayOfWeek === dayNum && selection?.session === session;
                        const selectable = available && filled;
                        const cellBg = !available
                          ? 'bg-gray-300'
                          : isSelected
                            ? 'bg-yellow-100'
                            : dayStripe;
                        ...
                        return (
                          <tr
                            key={`${dayNum}-${session}`}
                            onClick={() => handleSelectRow(dayNum, session, selectable)}
                            className={`border-b border-gray-300 ${
                              selectable ? 'cursor-pointer hover:bg-yellow-50' : ''
                            } ${isSelected ? 'ring-2 ring-inset ring-yellow-400' : ''}`}
                            title={
                              selectable
                                ? 'Click to select this workout'
                                : filled
                                  ? undefined
                                  : available
                                    ? 'Empty slot — assign a workout in Weekly Structure first'
                                    : 'Locked until previous workout is assigned'
                            }
                          >
                            {session === 1 && (
                              <td
                                className={`p-2 border border-gray-300 align-top font-semibold text-gray-900 ${
                                  selection?.dayOfWeek === dayNum ? 'bg-yellow-100' : dayStripe
                                }`}
                                rowSpan={3}
                              >
                                {dayName}
                            ...
                            <td
                              className={`p-2 border border-gray-300 align-middle whitespace-nowrap ${cellBg}`}
                            >
            ...
            <p className="text-xs text-gray-500">
              Target slot: Workout #{targetWorkout.sessionNumber} in your current day. Click a row
              with an assigned workout (green dot), then press Import.
            </p>
          ...
          <div className="px-5 py-3 border-t bg-gray-50 flex items-center justify-between shrink-0 gap-3">
            <p className="text-sm text-gray-500 truncate">
              {selectionLabel ?? 'No workout selected'}
            </p>
