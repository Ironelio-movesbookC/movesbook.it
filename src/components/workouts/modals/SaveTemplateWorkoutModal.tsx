...
import SaveTemplateWorkoutMetadataModal from './SaveTemplateWorkoutMetadataModal';
import SaveTemplateWorkoutToYearlyModal from './SaveTemplateWorkoutToYearlyModal';

type SaveMode = 'favorite' | 'archive' | 'yearly';

type PeriodOption = { id: string; name: string };

interface SaveTemplateWorkoutModalProps {
  isOpen: boolean;
  onClose: () => void;
  sourceWorkout: any;
  sourceDay: any;
...
const SAVE_MODES: Array<{
  ...
  },
  {
    ...
    label: (
      <>
        Save in <strong>Yearly Plan</strong> (select the week and the day)
      </>
    ),
    Icon: YearlyModeIcon,
  },
...
export default function SaveTemplateWorkoutModal({
  isOpen,
  onClose,
  sourceWorkout,
  sourceDay,
  ...
  if (activeMode === 'yearly') {
    return (
      <SaveTemplateWorkoutToYearlyModal
        isOpen={isOpen}
        onClose={onClose}
  ...
  if (activeMode === 'favorite' || activeMode === 'archive') {
    return (
      <SaveTemplateWorkoutMetadataModal
        isOpen={isOpen}
        onClose={onClose}
        onBack={() => setActiveMode(null)}
        sourceWorkout={sourceWorkout}
        ...
        >
          <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-5 py-3 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-lg font-bold">Save workout of the template weekly plan in..</h2>
            </div>
          ...
          <div className="p-5 overflow-y-auto flex-1">
            <p className="text-sm text-gray-600 mb-4">
              ...
              {sourceDay?.weekNumber != null ? ` — Week ${sourceDay.weekNumber}` : null}
            </p>
