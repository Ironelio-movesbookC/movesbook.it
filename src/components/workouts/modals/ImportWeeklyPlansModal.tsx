...
import ImportTemplateWeeklyPlanModal from './ImportTemplateWeeklyPlanModal';
import ImportStructureWeeklyPlanModal from './ImportStructureWeeklyPlanModal';
import ImportFavoriteWeeklyPlanModal from './ImportFavoriteWeeklyPlanModal';
import ImportCurrentYearlyPlanModal from './ImportCurrentYearlyPlanModal';
import ImportGeneralArchiveWeeklyPlanModal from './ImportGeneralArchiveWeeklyPlanModal';
import ImportCoachAnnualPlanModal from './ImportCoachAnnualPlanModal';
import ImportSharedUsersWeeklyPlanModal from './ImportSharedUsersWeeklyPlanModal';
import ImportSharedFriendsWeeklyPlanModal from './ImportSharedFriendsWeeklyPlanModal';

export type ImportSourceKind =
  | { type: 'template'; section: 'A' | 'B' | 'C'; weekNumbers: number[] }
  | {
      type: 'plan_weeks';
      planType: 'ARCHIVE' | 'WORKOUTS_DONE' | 'YEARLY_PLAN';
      weekIds: string[];
    }
  | { type: 'favorite'; favoriteIds: string[] }
  | {
      type: 'structure';
      items: Array<{ key: WeeklyStructurePlanKey; planData: WeeklyStructurePlanPersist }>;
    }
  | { type: 'global_archive'; entryIds: string[] }
  | {
      type: 'general_archive';
      source: { kind: 'personal'; weekId: string } | { kind: 'global'; entryId: string };
  ...
  | { type: 'yearly_plan_copy'; sourceWeekId: string; targetWeekIds: string[] };

export type ImportWeeklyPlansPayload = {
...
interface ImportWeeklyPlansModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const IMPORT_MODES: Array<{ id: ImportMode; label: string; Icon: () => React.ReactNode }> = [
  {
    id: 'templates',
    label: 'Import from Templates weekly plans',
    Icon: TemplateModeIcon,
  },
  {
    id: 'structure',
    label: 'Import from Weekly workouts structures',
    Icon: StructureModeIcon,
  },
  {
    id: 'favorite',
    label: 'Import from Favourites',
    Icon: FavoritesModeIcon,
  },
  {
    id: 'yearly',
    label: 'Import from Current Yearly Plan',
    Icon: YearlyModeIcon,
  },
  {
    id: 'general_archive',
    label: 'Import from General Archive',
    Icon: GeneralArchiveModeIcon,
  },
  {
    id: 'coach_annual',
    label: "Import from Your Coach's Annual Plan",
    Icon: CoachModeIcon,
  },
  {
    id: 'shared_users',
    label: 'Import from Weekly plans shared by users',
    Icon: SharedUsersModeIcon,
  },
  {
    id: 'shared_friends',
    label: 'Import from Plans shared by your friends',
    Icon: SharedFriendsModeIcon,
  },
];
      ...
      {[0, 1, 2].map((row) => (
        <div key={row} className="grid grid-cols-3 border-t border-blue-200">
          {[0, 1, 2].map((col) => (
            <div
              key={col}
              className={`border-r border-blue-100 ${row === 1 && col === 1 ? 'bg-orange-300' : 'bg-orange-50'}`}
            />
          ))}
        </div>
      ))}
...
export default function ImportWeeklyPlansModal({
  isOpen,
  onClose,
  anchorWeek,
  yearlyWeeks,
  onConfirm,
}: ImportWeeklyPlansModalProps) {
  const [showTemplateImport, setShowTemplateImport] = useState(false);
  const [showStructureImport, setShowStructureImport] = useState(false);
  const [showFavoriteImport, setShowFavoriteImport] = useState(false);
  const [showYearlyImport, setShowYearlyImport] = useState(false);
  const [showGeneralArchiveImport, setShowGeneralArchiveImport] = useState(false);
  const [showCoachAnnualImport, setShowCoachAnnualImport] = useState(false);
  const [showSharedUsersImport, setShowSharedUsersImport] = useState(false);
  const [showSharedFriendsImport, setShowSharedFriendsImport] = useState(false);

  useEffect(() => {
    if (!isOpen) return;
    setShowTemplateImport(false);
    setShowStructureImport(false);
    setShowFavoriteImport(false);
    setShowYearlyImport(false);
    setShowGeneralArchiveImport(false);
    setShowCoachAnnualImport(false);
    setShowSharedUsersImport(false);
    setShowSharedFriendsImport(false);
  ...
  const handleModeSelect = (mode: ImportMode) => {
    ...
    if (mode === 'favorite') {
      setShowFavoriteImport(true);
      return;
    }
    if (mode === 'yearly') {
      setShowYearlyImport(true);
      return;
    }
    if (mode === 'general_archive') {
      setShowGeneralArchiveImport(true);
      return;
    }
    if (mode === 'coach_annual') {
      setShowCoachAnnualImport(true);
      return;
    }
    if (mode === 'shared_users') {
      setShowSharedUsersImport(true);
      return;
    }
    if (mode === 'shared_friends') {
      setShowSharedFriendsImport(true);
      return;
    }
  };
  ...
  if (showSharedFriendsImport) {
    return (
      <ImportSharedFriendsWeeklyPlanModal
        isOpen={isOpen}
        onClose={onClose}
        onBack={() => setShowSharedFriendsImport(false)}
        anchorWeek={anchorWeek}
        yearlyWeeks={yearlyWeeks}
        onConfirm={onConfirm}
      />
    );
  }

  if (showSharedUsersImport) {
    return (
      <ImportSharedUsersWeeklyPlanModal
        isOpen={isOpen}
        onClose={onClose}
        onBack={() => setShowSharedUsersImport(false)}
        anchorWeek={anchorWeek}
        yearlyWeeks={yearlyWeeks}
        onConfirm={onConfirm}
      />
  ...
  if (showCoachAnnualImport) {
    return (
      <ImportCoachAnnualPlanModal
        isOpen={isOpen}
        onClose={onClose}
        onBack={() => setShowCoachAnnualImport(false)}
        anchorWeek={anchorWeek}
        yearlyWeeks={yearlyWeeks}
        onConfirm={onConfirm}
      />
  ...
  if (showGeneralArchiveImport) {
    return (
      <ImportGeneralArchiveWeeklyPlanModal
        isOpen={isOpen}
        onClose={onClose}
        onBack={() => setShowGeneralArchiveImport(false)}
        anchorWeek={anchorWeek}
        yearlyWeeks={yearlyWeeks}
        onConfirm={onConfirm}
      />
  ...
  if (showYearlyImport) {
    return (
      <ImportCurrentYearlyPlanModal
        isOpen={isOpen}
        onClose={onClose}
        onBack={() => setShowYearlyImport(false)}
        anchorWeek={anchorWeek}
        yearlyWeeks={yearlyWeeks}
        onConfirm={onConfirm}
      />
  ...
  if (showFavoriteImport) {
    return (
      <ImportFavoriteWeeklyPlanModal
        isOpen={isOpen}
        onClose={onClose}
        onBack={() => setShowFavoriteImport(false)}
        anchorWeek={anchorWeek}
        yearlyWeeks={yearlyWeeks}
        onConfirm={onConfirm}
      />
  ...
  if (showStructureImport) {
    return (
      <ImportStructureWeeklyPlanModal
        isOpen={isOpen}
        onClose={onClose}
          ...
          <div className="p-5 overflow-y-auto flex-1">
            <div className="space-y-4">
              ...
              <p className="text-xs text-gray-500">
                Importing into Yearly Plan from Week {anchorWeek.weekNumber} onward.
              </p>
            </div>
          </div>
