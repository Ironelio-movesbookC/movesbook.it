...
import { X, Settings, RefreshCw, CheckCircle, Clock, ChevronUp, ChevronDown, GripVertical, ChevronLeft, Info } from 'lucide-react';
...
type FastPlanMode = 'hide-exercises' | 'select-exercises' | 'plan-series';
...
export default function PlanGymWeekFastPlanModal({
  ...
  const [fastPlanMode, setFastPlanMode] = useState<FastPlanMode>('plan-series');
  ...
  const prevFastPlanModeRef = useRef<FastPlanMode>('plan-series');
  ...
  const handleAutoProcess = () => {
    // Always show the warning â€” button fills values every time regardless of manual edits
    ...
    if (inCurrentDay) {
      ...
      setFastPlanToolbarNotice(
        isFilter
          ...
          : `Showing ${mg.label} only â€” tap ${mg.label} again or ALL to show every area.`,
      );
    } else {
      ...
      setFastPlanToolbarNotice(`${mg.label} added â€” scroll down to fill exercise names or totals for this area.`);
    }
  };

  const sectionTabHint =
    sectionTab === 'execution'
      ? 'Execution â€” Edit tempo (speed), reps/time, weight and mode in each row.'
      : sectionTab === 'intensity'
        ? 'Intensity â€” Focus on load (weight), speed and effort mode.'
        : 'Break â€” Edit rests in the Break column; macros under each sector set pauses after exercises or the whole area.';
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
          ...
          <div className="p-5 space-y-4">
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-xs text-amber-800 space-y-1.5">
              ...
              <p>ðŸ“… Open your <strong>Yearly Plan</strong>, select the target week, and apply each routine to the desired days.</p>
              <p>ðŸ’¾ Your plan is saved locally as a <strong>template</strong> â€” reopen it anytime from the Plan Gym Week menu.</p>
            </div>
  ...
  const routineNameEl = (
    <div className="flex flex-wrap items-center gap-x-4 gap-y-2 px-0 py-1.5 bg-white">
      ...
      <div className="flex flex-wrap items-end gap-3 text-xs">
        ...
        <p className="max-w-xs text-[10px] leading-snug text-gray-500 pb-0.5">
          Period {clampedPeriod.currentPeriod} of {clampedPeriod.totalPeriods} â€” series, reps and pauses interpolate from your workout parameters (From â†’ To).
        </p>
      </div>
    </div>
  );
    ...
    return (
      <div ref={sortable?.setSortableRef} style={sortable?.style}>
        ...
        >
          ...
          <div
            ...
            title={
              fastPlanMode === 'select-exercises'
                ...
                : 'Add or remove exercises (stations). Max 2Ã— default from the distribution table; min 1. With â€œKeep fix this valueâ€, total series stays the same while counts change.'
            }
          >
            <button
              type="button"
            ...
            {openShuffleMenuFor === sec.sectorId ? (
              <div className="absolute left-0 top-9 z-30 w-44 rounded-lg border border-gray-200 bg-white p-1 shadow-lg">
                <button
                  ...
                  title="Assign random exercises from this muscleâ€™s catalog to every row, then randomize series order across rows"
                >
                  Random mode
                </button>
                <button
          ...
          <div className="flex flex-wrap items-center gap-3 border-l border-cyan-200/80 pl-2 text-xs">
            <label className="flex cursor-pointer items-center gap-1.5 text-gray-600">
              <input
                type="radio"
                name={`planMode-${activeDayIdx}`}
                className="h-3 w-3 accent-blue-600"
                checked={fastPlanMode === 'hide-exercises'}
                onChange={() => setFastPlanMode('hide-exercises')}
              />
              Hide exercises
            </label>
            <label className="flex cursor-pointer items-center gap-1.5 text-gray-700">
              <input
                ...
                onChange={() => {
                  ...
                  setFastPlanCatalogFilterId(sec.sectorId);
                }}
              />
              Select exercises
            </label>
            <label className="flex cursor-pointer items-center gap-1.5 font-medium text-gray-800">
              <input
                type="radio"
                name={`planMode-${activeDayIdx}`}
                className="h-3 w-3 accent-blue-600"
          ...
          <div className="border-b border-yellow-200 bg-yellow-50/95 px-2 py-2 shadow-inner">
            <div className="mb-2 flex flex-wrap items-end gap-2">
              ...
              <label className="flex min-w-[180px] flex-col gap-0.5 text-[10px] font-semibold text-gray-800">
                ...
                >
                  <option value="">Free movesâ€¦</option>
        ...
        {fastPlanMode !== 'hide-exercises' ? sec.exercises.map((ex, exIdx) => {
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            <div className="bg-amber-500 px-5 py-4 flex items-start gap-3">
              <span className="text-white text-2xl leading-none mt-0.5">âš ï¸</span>
              <div>
            ...
            <div className="px-5 py-4 space-y-3">
              ...
              <ul className="text-xs text-gray-700 list-disc list-inside space-y-0.5 pl-1">
                <li>Speed â†’ Normal</li>
                <li>Series â†’ calculated by distribution table</li>
                <li>Rip/Time â†’ from sector reps setting</li>
                <li>Break â†’ from sector pause setting</li>
                <li>Mode â†’ Stopped (default)</li>
              </ul>
