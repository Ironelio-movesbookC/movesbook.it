...
import {
  ...
  getSectorColor,
  SeriesDistDialog,
  EMPTY_DIST_CONSTANT_SECTOR_IDS,
...
function newPlanRowId(): string {
  return typeof crypto !== 'undefined' && crypto.randomUUID
    ? crypto.randomUUID()
    : `pr-${Date.now()}-${Math.random().toString(36).slice(2, 11)}`;
}

export interface PlanSector {
  /** Stable key for drag-and-drop and React list identity (unique per row). */
  planRowId:           string;
  sectorId:            string;
  sectorLabel:         string;
  image:               string;
...
/** Tab toolbar + grid: subtle column emphasis so Execution / Intensity / Break feel “live”. */
function toneCell(
  tab: 'execution' | 'intensity' | 'break',
  col: 'speed' | 'series' | 'rips' | 'weight' | 'brk' | 'mode',
): string {
  if (tab === 'execution' && (col === 'speed' || col === 'series' || col === 'rips')) return ' bg-emerald-50/70';
  if (tab === 'intensity' && (col === 'speed' || col === 'weight' || col === 'mode')) return ' bg-amber-50/70';
  if (tab === 'break' && col === 'brk') return ' bg-sky-50/70';
  return '';
}
...
function initPlanDay(day: ManualDayPlan, levelCat: SeriesLevelCategory = 'mid'): PlanDay {
  return {
    ...
    sectors: day.sectors.map((rawSec) => {
      ...
      const fromTable = dist.length;
      /** Manual Plan Gym Week `exercises` (when > 0) wins — matches operator intent when opening Fast Plan. */
      const planned = Math.max(0, Math.round(Number(sec.exercises) || 0));
      let count: number;
      if (sec.series <= 0) {
        count = 0;
      } else if (planned > 0) {
        count = Math.max(1, Math.min(planned, sec.series));
      } else {
        count = fromTable;
      }
...
/** Planned series weight for % shares when opening SeriesDistDialog (fallbacks keep UI and dialog aligned). */
function plannedSeriesWeightForDistDialog(s: PlanSector): number {
  const ts = Math.max(0, Math.round(Number(s.totalSeries) || 0));
  if (ts > 0) return ts;
  const def = Math.max(0, Math.round(Number(s.defaultTotalSeries) || 0));
  if (def > 0) return def;
  return s.exercises.reduce(
    (sum, ex) => sum + Math.max(0, Math.round(Number(ex.distributedSeries) || 0)),
    0,
  );
}
...
function planDaysToManual(days: PlanDay[]): ManualDayPlan[] {
  return days.map(d => ({
    ...
    sectors: d.sectors.map(s => ({
      ...
      series:           plannedSeriesWeightForDistDialog(s),
      reps:             s.reps,
      pause:            s.pause,
...
/** Wraps one muscular-area block; drag handle uses sortable listeners only. */
function SortableSectorWrap({
  id,
  children,
}: {
  id: string;
  ...
  const style: React.CSSProperties = {
    ...
    transition,
    ...(isDragging
      ? { zIndex: 40, position: 'relative', boxShadow: '0 12px 28px rgba(0,0,0,0.14)' }
      : {}),
  };
  const dragHandleProps = { ...attributes, ...listeners } as React.HTMLAttributes<HTMLButtonElement>;
  return <>{children({ setSortableRef: setNodeRef, style, dragHandleProps, isDragging })}</>;
}
...
interface PlanGymWeekFastPlanModalProps {
  ...
  /** Return to Plan Gym Week manual sector screen with current totals (series/exercises per area). */
  onBack?:         (plan: PlanGymWeekManualResult) => void;
}

// ─── Component ────────────────────────────────────────────────────────────────
export default function PlanGymWeekFastPlanModal({
  isOpen, plan, goals = [], trainingLevel, onClose, onSave, onBack,
        ...
        if (!s.keepSeriesFixed) {
          ...
          if (newCount > exercises.length) {
            const add = Array.from({ length: newCount - exercises.length }, (_, i) => ({
              ...
              breakTime: s.pause || "1'", mode: 'Stopped', userEdited: false,
            }));
  ...
  const requestRemoveExercise = useCallback((secIdx: number, exIdx: number) => {
    const day = planDays[activeDayIdx];
    const sec = day?.sectors[secIdx];
    if (!sec || sec.exercises.length <= 1) return;
    if (!window.confirm(`Remove this exercise from ${sec.sectorLabel}?`)) return;
    removeExerciseAt(secIdx, exIdx);
  }, [activeDayIdx, planDays, removeExerciseAt]);

  const requestRemoveSector = useCallback((secIdx: number) => {
    const day = planDays[activeDayIdx];
    const sec = day?.sectors[secIdx];
    if (!sec) return;
    if (day.sectors.length <= 1) {
      window.alert('At least one muscular area is required for this day.');
      return;
    }
    if (!window.confirm(`Remove "${sec.sectorLabel}" and all exercises in this area from this day?`)) return;
    const deletedSectorId = sec.sectorId;
    setPlanDays(prev => prev.map((d, di) => {
      if (di !== activeDayIdx) return d;
      return { ...d, sectors: d.sectors.filter((_, i) => i !== secIdx) };
    }));
    setFilterSector(fs => (fs === deletedSectorId ? null : fs));
  }, [activeDayIdx, planDays]);

  const sectorDnDSensors = useSensors(
  ...
  const handleSectorDragEnd = useCallback((event: DragEndEvent) => {
    ...
    setPlanDays(prev => prev.map((d, di) => {
      if (di !== activeDayIdx) return d;
      const oldIndex = d.sectors.findIndex(s => s.planRowId === active.id);
      const newIndex = d.sectors.findIndex(s => s.planRowId === over.id);
      if (oldIndex < 0 || newIndex < 0) return d;
      return { ...d, sectors: arrayMove(d.sectors, oldIndex, newIndex) };
    }));
  }, [activeDayIdx]);
  ...
  const applyAutoProcess = useCallback(() => {
    setShowAutoWarn(false);
    const goalId = goals[activeDayIdx];
    const goalSettings = readGoalParamsFromWorkoutSettings(goalId);
    const y = plan.yearlyPeriodSettings;
    const curPeriod = Math.max(1, Math.floor(y?.currentPeriod ?? 1));
    const totalPeriods = Math.max(curPeriod, Math.floor(y?.totalPeriods ?? 1));
    ...
    const { defaultReps, defaultPauseLabel, defaultMacroExerciseLabel, defaultMacroEndSectorLabel } = scalars;

    setPlanDays((prev) =>
      prev.map((d, di) => {
        if (di !== activeDayIdx) return d;
        const sectors = d.sectors.map((s) => {
          const exercises = clearExerciseDistributedSeries(s.exercises).map((ex) => ({
            ...ex,
            speed: 'Normal',
            ripsTime: String(defaultReps),
            weight: '',
            breakTime: defaultPauseLabel,
            mode: 'Stopped',
            userEdited: false,
          }));
          const ts = Math.max(0, s.totalSeries);
          const seriesReps = Array.from({ length: ts }, () => defaultReps);
          return {
            ...s,
            reps: defaultReps,
            pause: defaultPauseLabel,
            macroExercise: defaultMacroExerciseLabel,
            macroEndOfSector: defaultMacroEndSectorLabel,
            seriesReps,
            exercises,
          };
        });
  ...
  const routineNameEl = (
    <div className="flex-shrink-0 flex flex-wrap items-center gap-2 px-3 py-1.5 bg-white border-b border-gray-100">
      ...
      <button
        ...
        className="ml-auto shrink-0 text-xs font-semibold text-blue-700 underline underline-offset-2 hover:text-blue-900"
      >
        {planDetailsExpanded ? 'Hide details' : 'Show details'}
      </button>
    </div>
  );
    ...
    <>
      {planDetailsExpanded ? (
        <>
          ...
          {/* Toolbar stays above sector blocks in paint order — avoids invisible hits from @dnd-kit sortable layers */}
          <div className="relative z-[5] isolate bg-white">
          <div className={`mx-3 flex ${planDetailsExpanded ? 'mt-3' : 'mt-2'}`}>
          ...
          <p className="mx-3 mt-2 rounded-lg border border-amber-200/90 bg-amber-50/95 px-3 py-2 text-[11px] leading-snug text-amber-950">
            {sectionTabHint}
          </p>
          {fastPlanToolbarNotice ? (
            <div className="mx-3 mt-2 rounded-lg border border-green-300 bg-green-50 px-3 py-2 text-xs font-medium text-green-950 shadow-sm" role="status" aria-live="polite">
              {fastPlanToolbarNotice}
            </div>
          ) : null}
          ...
          <div className="mx-3 rounded-xl border border-[#050a14] bg-[#0a1628] px-3 py-3 shadow-inner">
            <div className="flex items-stretch gap-2 sm:gap-3">
              <button type="button" onClick={onMuscleStripAll}
                ...
                className="touch-manipulation flex flex-shrink-0 cursor-pointer flex-col items-center justify-center gap-1 self-center select-none">
                  ...
                  {GYM_WEEK_MUSCLE_GROUPS.map((mg) => {
                    ...
                    return (
                      <button key={mg.id} type="button"
                        ...
                        className={`touch-manipulation flex w-[72px] flex-shrink-0 cursor-pointer flex-col items-center gap-1 rounded-lg border bg-white px-1.5 py-1.5 transition-shadow select-none ${
                          isFilter
                        ...
                        <div className="relative h-14 w-14 shrink-0 pointer-events-none">
                          <Image src={mg.image} alt="" draggable={false} fill className="object-contain" unoptimized />
                        </div>
      ...
      <div
        className={`relative z-0 mx-3 mb-4 overflow-hidden border border-gray-300 ${
          planDetailsExpanded ? 'rounded-b-lg border-t-0' : 'mt-3 rounded-lg'
        }`}
      >
        {!filterSector ? (
          <DndContext sensors={sectorDnDSensors} collisionDetection={closestCenter} onDragEnd={handleSectorDragEnd}>
            <SortableContext items={activeDay.sectors.map(s => s.planRowId)} strategy={verticalListSortingStrategy}>
              {activeDay.sectors.map((sec, secIdx) => (
                <SortableSectorWrap key={sec.planRowId} id={sec.planRowId}>
          ...
          visibleSectors.map(sec => {
            const secIdx = activeDay.sectors.findIndex(s => s.planRowId === sec.planRowId);
            return (
              <React.Fragment key={sec.planRowId}>
      ...
      <div className="flex flex-wrap items-center gap-2">
        {onBack ? (
          <button
            ...
            className="flex items-center gap-1 rounded-lg border border-gray-400 bg-white px-4 py-2 text-sm font-semibold text-gray-800 hover:bg-gray-100"
          >
              ...
              <div className="flex shrink-0 items-center gap-1">
                {onBack ? (
                  <button
                    type="button"
                    onClick={handleBackToManual}
                    className="rounded-lg px-2 py-1 text-sm font-semibold text-gray-700 hover:bg-gray-200 hover:text-gray-900"
                  >
                    Back
                  </button>
                ) : null}
                <button type="button" onClick={onClose} className="p-1 rounded-lg hover:bg-gray-200 text-gray-500" aria-label="Close">
                  <X className="w-5 h-5" />
                </button>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            ...
            <div className="px-5 py-4 space-y-3">
              <p className="text-sm text-gray-800 leading-relaxed">
                This restores defaults from <strong>Workouts parameters settings</strong> (repetitions and pauses among exercises, series, and areas), interpolated by <strong>training level</strong> (Beginner→Professional in five steps) and your <strong>yearly period</strong>.
              </p>
              <p className="text-xs text-gray-600 leading-relaxed">
                <strong>Kept:</strong> your exercise rows — same count per sector and each exercise name.{' '}
                <strong>Reset:</strong> Rip/Time, Break between rows, Speed, Mode, Weight, sector repetitions, and macro pauses (among series / end of area) to the calculated values.
              </p>
              <p className="text-xs text-amber-700 font-medium bg-amber-50 border border-amber-200 rounded px-3 py-2">
                Run this anytime after manual edits; only exercise choices stay — everything else is recalculated.
              </p>
            </div>
