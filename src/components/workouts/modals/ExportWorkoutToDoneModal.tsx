...
interface ExportWorkoutToDoneModalProps {
  isOpen: boolean;
  onClose: () => void;
  sourceWorkout: any;
...
export default function ExportWorkoutToDoneModal({
  isOpen,
  onClose,
  sourceWorkout,
  onConfirm,
}: ExportWorkoutToDoneModalProps) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        const weeks = await fetchPlanWeeks(token, 'WORKOUTS_DONE');
        setDoneWeeks(mergeWeeksByWeekNumber(weeks));
      } catch {
        setDoneWeeks([]);
      } finally {
        setIsLoading(false);
  ...
  const selectedWeek = useMemo(
    () => doneWeeks.find((w) => w.id === selectedWeekId),
    [doneWeeks, selectedWeekId]
  );
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 flex items-center justify-center z-[999999] p-4">
        ...
        >
          <div className="bg-gradient-to-r from-emerald-600 to-teal-700 text-white px-6 py-4 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2">
              <CheckCircle2 size={20} />
              <h2 className="text-lg font-bold">Export Workout to Workouts Done</h2>
            </div>
          ...
          <div className="p-6 overflow-y-auto flex-1 space-y-4">
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
              ...
              <p className="text-xs text-gray-600 mt-1">{mfCount} moveframe(s) — pick one Done week and up to 7 days.</p>
            </div>
            ...
            <div>
              ...
              {isLoading ? (
                ...
                >
                  <option value="">Choose Done week…</option>
                  {doneWeeks.map((week, idx) => (
                ...
                <div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto">
                  {weekDays.map((day: any) => {
                    ...
                    return (
                      ...
                      >
                        {isSelected ? (
                          <CheckSquare className="w-4 h-4 shrink-0 mt-0.5 text-teal-600" />
                        ) : (
