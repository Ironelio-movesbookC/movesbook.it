...
interface ImportSharedFriendsWeeklyPlanModalProps {
  isOpen: boolean;
  onClose: () => void;
  onBack: () => void;
...
export default function ImportSharedFriendsWeeklyPlanModal({
  isOpen,
  onClose,
  onBack,
  anchorWeek,
  yearlyWeeks,
  onConfirm,
}: ImportSharedFriendsWeeklyPlanModalProps) {
        ...
        >
          ...
          <div className="p-4 sm:p-5 overflow-y-auto flex-1 space-y-4">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3 py-2 px-4 bg-white border border-gray-200 rounded-xl">
                ...
                <span className="font-semibold text-gray-900">
                  Import from <span className="text-violet-700">Plans shared by your friends</span>
                </span>
              </div>
            ...
            <div className="bg-violet-50 border border-violet-200 rounded-lg p-3 text-sm text-violet-900">
              The weekly plans shown here are ones your friends made shareable in their{' '}
              <strong>General Archive of Workouts</strong>. They must copy or export a weekly plan
              to their Archive and tag it as <strong>shareable</strong>. This works like{' '}
              <strong>Weekly plans shared by users</strong>, but only your friends&apos; plans appear
              here.
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <select
                ...
                title="Select friend"
              >
                <option value="all">Select friend</option>
                {friends.map((friend) => (
            ...
            {showFilters && (
              <p className="text-xs text-gray-600 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
                Only shareable weekly plans from your friends appear here. Filter by friend, sport
                category, language, tags, or search by title / username.
              </p>
            )}
            ...
            {loading ? (
              <p className="text-center py-10 text-gray-500 text-sm">Loading friends&apos; plans…</p>
            ) : filteredRecords.length === 0 ? (
              <p className="text-center py-10 text-amber-800 bg-amber-50 border border-amber-200 rounded-lg text-sm">
                No shareable weekly plans from your friends yet. Friends must export a week to their
