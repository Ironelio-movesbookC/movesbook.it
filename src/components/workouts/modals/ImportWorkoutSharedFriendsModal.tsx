...
import { isSeriesBasedSport, getSportFastPlanningCategory } from '@/constants/moveframe.constants';
import { getSportDisplayName } from '@/lib/personalArchiveGridMapper';
...
interface ImportWorkoutSharedFriendsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onBack: () => void;
  targetDay: { id: string };
...
function sharedByLabel(record: WorkoutArchiveGridRecord): string {
  ...
  const user = record.sharedByUsername ?? record.authorFullName ?? 'Friend';
...
export default function ImportWorkoutSharedFriendsModal({
  isOpen,
  onClose,
  onBack,
  targetDay,
  targetWorkout,
  onConfirm,
}: ImportWorkoutSharedFriendsModalProps) {
  ...
  const loadRecords = useCallback(async () => {
    ...
    try {
      ...
      const params = new URLSearchParams({ recordType: 'WORKOUT' });
      if (language !== 'all') params.set('language', language);
      if (selectedFriend !== 'all') params.set('friendId', selectedFriend);

      const res = await fetch(
        '/api/workouts/shared-friends-weekly-plans?' + params.toString(),
      ...
      setRecords(
        (data.records ?? []).map((r: WorkoutArchiveGridRecord) => ({
          ...r,
  ...
  const allTags = useMemo(
    () =>
      Array.from(new Set(records.flatMap((r) => parseTags(r.tags))))
        .filter((t) => t.toLowerCase() !== 'shareable')
        .sort(),
    [records]
  );
  ...
  const handleImport = async () => {
    ...
    try {
      await onConfirm({
        ...
        source: { type: 'global_archive', globalEntryId: selectedRecordId },
      });
      onClose();
    } catch (e) {
    ...
    return (
      <div className="bg-violet-50 border border-violet-200 rounded-lg px-3 py-2">
        ...
        {previewMoveframes.length === 0 ? (
          <p className="text-xs text-gray-500 py-2">No moveframes in this shared workout.</p>
        ) : (
      ...
      record.totalTimeSeconds && record.totalTimeSeconds > 0
        ...
        : record.totalMeters
          ...
          : record.totalSeries
            ...
            : '—';
    const displayTags = parseTags(record.tags).filter((t) => t.toLowerCase() !== 'shareable');
      ...
      >
        ...
        {viewMode === 'table' && (
          <p className="text-[10px] text-gray-500 mt-2 truncate">
            ...
            {formatCreated(record.sharedAt ?? record.createdAt)}
          </p>
        )}
      </div>
    );
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 flex items-center justify-center z-[999999] p-4">
        ...
        >
          <div className="bg-gradient-to-r from-purple-600 to-violet-600 text-white px-5 py-3 flex items-center justify-between shrink-0">
          ...
          <div className="p-4 sm:p-5 overflow-y-auto flex-1 space-y-4">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-3 py-2 px-4 bg-white border border-gray-200 rounded-xl flex-1 min-w-[200px]">
                <Users className="w-8 h-8 text-violet-600 shrink-0" />
                <span className="font-semibold text-gray-900">
                  Import from{' '}
                  <span className="text-violet-700">shared by your friends</span>
                </span>
              </div>
            ...
            <div className="bg-violet-50 border border-violet-200 rounded-lg p-3 text-sm text-violet-900">
              The workouts shown here are ones your friends made shareable in their{' '}
              <strong>General Archive of Workouts</strong>. They must copy or export a workout to
              their Archive and tag it as <strong>shareable</strong>. This works like{' '}
              <strong>Workouts shared by users</strong>, but only your friends&apos; workouts appear
              here.
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <select
                ...
                className="px-3 py-2 border border-gray-300 rounded-lg text-sm min-w-[140px] focus:outline-none focus:ring-2 focus:ring-violet-500"
              >
            ...
            {showFilters && (
              <div className="flex flex-wrap items-center gap-3 text-xs text-gray-600 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
                <span>
                  Only shareable workouts from friends&apos; General Archives. Filter by friend,
                  sport category, search, or custom tags.
                </span>
                <select
                  value={language}
                  onChange={(e) => setLanguage(e.target.value)}
                  className="px-2 py-1 border border-gray-300 rounded text-xs ml-auto"
                >
            ...
            {loading ? (
              <p className="text-center py-10 text-gray-500 text-sm">Loading friends&apos; workouts…</p>
            ) : filtered.length === 0 ? (
              <p className="text-center py-10 text-amber-800 bg-amber-50 border border-amber-200 rounded-lg text-sm">
                No shareable workouts from your friends yet. Friends must export a workout to their
