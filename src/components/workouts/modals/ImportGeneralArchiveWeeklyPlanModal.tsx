...
import { fetchPlanWeeks, isWeekEmpty } from '@/lib/workoutPlanLoad';
...
interface ImportGeneralArchiveWeeklyPlanModalProps {
  isOpen: boolean;
  onClose: () => void;
  onBack: () => void;
...
function getTrainingDays(record: WorkoutArchiveGridRecord): number {
  const raw = record._raw as { days?: Array<{ workouts?: unknown[] }> } | undefined;
  if (raw?.days?.length) {
    const active = raw.days.filter((d) => (d.workouts?.length ?? 0) > 0).length;
    return active > 0 ? active : raw.days.length;
  }
  return 7;
}

function getPersonalWeekId(record: WorkoutArchiveGridRecord): string | null {
  if (record.archiveSource !== 'personal') return null;
  const raw = record._raw as { id?: string } | undefined;
  if (raw?.id) return raw.id;
  if (record.id.startsWith('week-')) return record.id.slice(5);
  return null;
}

function getPreviewWeek(record: WorkoutArchiveGridRecord): any | null {
  if (record.archiveSource === 'personal') {
    const raw = record._raw;
    if (raw && typeof raw === 'object' && 'days' in (raw as object)) return raw;
  }
  return null;
}

export default function ImportGeneralArchiveWeeklyPlanModal({
  isOpen,
  onClose,
  onBack,
  anchorWeek,
  yearlyWeeks,
  onConfirm,
}: ImportGeneralArchiveWeeklyPlanModalProps) {
  ...
  const [viewPlan, setViewPlan] = useState<WorkoutArchiveGridRecord | null>(null);
  ...
  const anchorIndex = useMemo(() => {
    const byId = sortedYearlyWeeks.findIndex((w) => w.id === anchorWeek.id);
    if (byId !== -1) return byId;
    return sortedYearlyWeeks.findIndex((w) => w.weekNumber === anchorWeek.weekNumber);
  }, [sortedYearlyWeeks, anchorWeek.id, anchorWeek.weekNumber]);

  const maxConsecutiveWeeks =
    anchorIndex === -1 ? 0 : Math.max(0, sortedYearlyWeeks.length - anchorIndex);

  const targetWeeks = useMemo(() => {
    if (anchorIndex === -1 || consecutiveCount < 1) return [];
    return sortedYearlyWeeks.slice(anchorIndex, anchorIndex + consecutiveCount);
  ...
  const loadRecords = useCallback(async () => {
    ...
    try {
      ...
      const [archiveWeeks, globalRes] = await Promise.all([
        fetchPlanWeeks(token, 'ARCHIVE'),
        fetch('/api/workouts/global-archive?recordType=WEEKLY_PLAN', {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);

      const personal = mapPersonalArchiveToGridRecords({ weeks: archiveWeeks }).filter(
        (r) => r.recordType === 'WEEKLY_PLAN'
      );
  ...
  const filteredRecords = useMemo(() => {
    return records
      .filter((record) => {
        if (record.recordType !== 'WEEKLY_PLAN') return false;
  ...
  const copyTargetCount = selectedRecord ? targetWeeks.length : 0;
  ...
  const handleDuplicate = async (record: WorkoutArchiveGridRecord) => {
    const weekId = getPersonalWeekId(record);
    if (!weekId) {
      alert('Duplicate is available for weeks in your personal archive only.');
      return;
    }
    const name = prompt('Name for the copy:', `${record.title} (copy)`);
    if (!name?.trim()) return;
    try {
      const token = localStorage.getItem('token');
      if (!token) return;
      const createRes = await fetch('/api/workouts/archive/weeks', {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!createRes.ok) throw new Error('Failed to create archive week');
      const { week: newWeek } = await createRes.json();
      const copyRes = await fetch('/api/workouts/weeks/copy', {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ sourceWeekId: weekId, targetWeekId: newWeek.id }),
      });
      if (!copyRes.ok) {
  ...
  const handleCopy = async () => {
    ...
    const targetWeekIds = targetWeeks.map((w) => w.id);
    let source:
      | { kind: 'personal'; weekId: string }
      | { kind: 'global'; entryId: string };

    const personalWeekId = getPersonalWeekId(selectedRecord);
    if (personalWeekId) {
      source = { kind: 'personal', weekId: personalWeekId };
    } else if (selectedRecord.archiveSource === 'global') {
      source = { kind: 'global', entryId: selectedRecord.id };
    } else {
      alert('Unable to import this record.');
      return;
    }
  ...
  const previewWeek = viewPlan ? getPreviewWeek(viewPlan) : null;
  ...
  const renderConsecutiveConfig = () => (
    <div className="space-y-3">
      <div>
        ...
        <div className="flex items-center gap-4">
          ...
          <p className="text-sm text-gray-700 flex-1">
            Copy to{' '}
            <span className="font-semibold text-blue-600">{consecutiveCount}</span> consecutive
            week(s) starting at Week {anchorWeek.weekNumber}. Maximum: {maxConsecutiveWeeks}{' '}
            week(s) available.
          </p>
        </div>
      </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm text-amber-900">
        <strong>Note:</strong> This will copy all workouts, moveframes, and movelaps from the source
      ...
      >
        <div className="flex items-start justify-between mb-3">
          ...
          <div className="flex gap-1 shrink-0">
            <button
              type="button"
              onClick={() =>
                alert('To edit archive metadata, open My Settings → General Archive.')
              }
              className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition"
              title="Edit in General Archive"
            >
            ...
            <button
              ...
              onClick={() =>
                alert(
                  record.archiveSource === 'personal'
                    ? 'Remove weeks from My Settings → General Archive.'
                    : 'Global archive entries can only be removed by administrators.'
                )
              }
        ...
        <div className="space-y-1.5 mb-4 text-sm">
          <div className="flex justify-between gap-2">
            ...
            <span className="font-semibold text-gray-900">{formatSavedFrom(record.createdAt)}</span>
          </div>
          <div className="flex justify-between gap-2">
          ...
          <div className="flex justify-between gap-2">
            ...
            <span className="font-semibold text-gray-900">{getTrainingDays(record)} days</span>
          </div>
          <div className="flex justify-between gap-2">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            title="Duplicate in archive"
          >
            <Copy className="w-4 h-4" />
          </button>
          <button
            type="button"
            onClick={() => setViewPlan(record)}
            className="px-3 py-2 border-2 border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition"
            title="Preview week"
            disabled={!getPreviewWeek(record)}
          >
    ...
    <>
      ...
      <div className="fixed inset-0 flex items-center justify-center z-[999999] p-4">
        ...
        >
          ...
          <div className="p-4 sm:p-5 overflow-y-auto flex-1 space-y-4">
            ...
            {showFilters && (
              <p className="text-xs text-gray-600 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
                Showing weekly plans from your personal archive and the shared Movesbook catalog.
                Use sport category buttons, search, and tags to narrow the list.
              </p>
            )}
            ...
            {loading ? (
              <p className="text-center py-10 text-gray-500 text-sm">Loading archive plans…</p>
            ) : filteredRecords.length === 0 ? (
              <p className="text-center py-10 text-amber-800 bg-amber-50 border border-amber-200 rounded-lg text-sm">
                No weekly plans found in your general archive. Save a week to the archive or browse
                the global catalog in My Settings → General Archive.
              </p>
            ) : viewMode === 'tree' ? (
                ...
                {filteredRecords.map((record) => {
                  ...
                  return (
                    <div key={record.id} className="space-y-2">
                      <div
                        className={`grid grid-cols-6 gap-2 items-center px-3 py-3 border rounded-lg text-sm ${
                          selected
          ...
          {selectedRecord && (
            <div className="border-t border-blue-200 bg-blue-50/60 px-5 py-4 shrink-0 space-y-3 max-h-[40vh] overflow-y-auto">
              ...
              {anyTargetHasContent && (
                <label className="flex items-start gap-3 p-3 border border-amber-300 bg-amber-50 rounded-lg cursor-pointer">
                  ...
                  <span className="text-sm text-amber-900">
                    ...
                    them with the imported archive plan.
                  </span>
                </label>
              )}
            </div>
          ...
          <div className="px-5 py-3 border-t bg-gray-50 flex items-center justify-between shrink-0">
            <p className="text-sm text-gray-500">
              {!selectedRecord
                ? 'No plan selected'
                : copyTargetCount > 0
                  ? `${copyTargetCount} target week(s) ready to copy`
                  : 'No target weeks available in your yearly plan'}
            </p>
            <div className="flex gap-2">
              <button
                type="button"
              ...
              >
                {isImporting
                  ? 'Copying…'
                  : `Copy to ${copyTargetCount} Week(s)`}
              </button>
            </div>
          </div>
        </div>
