...
function toTableRow(record: ProcedureRecordView, index: number, onDelete?: (id: string) => void): Member {
  const row: Member = {
    id: record.id,
    number: index + 1,
...
export default function ProcedureRecordsArchive({ procedureCode, activeTab }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await client.fetchRecords({ page, pageSize: client.pageSize });
      setTotal(res.total);
      setData(
        res.items.map((record, i) =>
          toTableRow(record, (page - 1) * client.pageSize + i, async (id) => {
