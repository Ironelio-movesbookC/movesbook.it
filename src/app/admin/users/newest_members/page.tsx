...
import type { NewestUserRow, NewestUsersPayload } from '@/lib/admin/buildNewestUsers';
import {
  STATS_KIND_LABELS,
  STATS_USER_KINDS,
  type StatsUserKind,
...
function GenderOrPhoto({ user }: { user: NewestUserRow }) {
  ...
  return (
    <div
      className={`flex h-10 w-10 shrink-0 items-center justify-center border border-[#ccc] bg-white text-xl font-bold ${accent}`}
    >
...
export default function AdminNewestMembersPage() {
  const router = useRouter();
  const [authOk, setAuthOk] = useState(false);
  const [userType, setUserType] = useState<StatsUserKind | 'all'>('all');
  const [country, setCountry] = useState('');
  const [data, setData] = useState<NewestUsersPayload | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const [msgOpen, setMsgOpen] = useState(false);
  const [msgUser, setMsgUser] = useState<NewestUserRow | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData((await res.json()) as NewestUsersPayload);
    } catch (e) {
  ...
  return (
    <div className="min-h-full bg-[#ececec] p-4 md:p-6">
      <div className="mb-4">
        ...
        <h1 className="mt-1 text-2xl font-bold text-[#222]">Newest Users</h1>
        <p className="mt-1 text-sm text-[#555]">
          Users registered in the last 60 days · sorted by subscription start (most recent first).
          Green = online · Orange = connected today
        </p>
      </div>
      ...
      <div className="border border-[#cfcfcf] bg-white">
        {loading ? (
          ...
          <div className="p-8 text-center text-[#888]">No newest users in the last 60 days.</div>
        ) : (
                ...
                <li key={user.id} className="flex items-start gap-3 px-4 py-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-[#666]">
                      Sub start: {user.subscriptionStart}
