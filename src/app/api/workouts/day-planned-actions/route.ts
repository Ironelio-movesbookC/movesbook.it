...
import type { Prisma, WorkoutPlanType } from '@prisma/client';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const planType = searchParams.get('planType') as WorkoutPlanType | null;
    ...
    const dayWhere: Prisma.WorkoutDayWhereInput = { userId: dbUserId };

    if (planType) {
      dayWhere.workoutWeek = {
        workoutPlan: {
          userId: dbUserId,
          type: planType,
        },
    ...
    const where: Prisma.WorkoutDayPlannedActionWhereInput = {
      userId: dbUserId,
      workoutDay: dayWhere,
      ...(templateId ? { templateId } : {}),
    };

    const orderBy: Prisma.WorkoutDayPlannedActionOrderByWithRelationInput[] =
      sortBy === 'action'
        ? [{ nameSnapshot: sortDir }, { workoutDay: { date: 'asc' } }]
        : [{ workoutDay: { date: sortDir } }, { sortOrder: 'asc' }];

    const rows = await prisma.workoutDayPlannedAction.findMany({
      where,
      orderBy,
      include: {
        workoutDay: {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!workoutDayId || !templateId) {
      return NextResponse.json(
        { error: 'workoutDayId and templateId are required' },
        { status: 400 }
      );
    }
    ...
    const maxOrder = await prisma.workoutDayPlannedAction.aggregate({
      where: { workoutDayId },
      _max: { sortOrder: true },
    });
    const sortOrder = (maxOrder._max.sortOrder ?? -1) + 1;

    const row = await prisma.workoutDayPlannedAction.create({
      data: {
        workoutDayId,
        userId: dbUserId,
        templateId,
