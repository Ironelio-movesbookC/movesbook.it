...
export async function POST(request: NextRequest) {
  ...
  const viewerUserId = verifyToken(token)?.userId;
  if (!viewerUserId) {
  ...
  const uploadDir = join(getServerPublicDir(), 'uploads', folder, viewerUserId);
  ...
  const publicPath = `/uploads/${folder}/${viewerUserId}/${fileName}`;
  const browserPath = toMediaApiPath(publicPath) ?? publicPath;

  return NextResponse.json({ success: true, kind, path: browserPath });
}
