...
/** Music has no built-in default topics; users create them via POST. */
const DEFAULT_TOPIC_NAMES: string[] = [];
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const allCustom = await prisma.userMusicTopic.findMany({
    ...
    console.error('GET /api/music/topics', e);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const count = await prisma.userMusicTopic.count({ where: { userId } });
    const created = await prisma.userMusicTopic.create({
    ...
    console.error('POST /api/music/topics', e);
