...
/**
 * GET /api/music/ogp-groups/likes?ids=id1,id2
...
export async function GET(request: NextRequest) {
  ...
  try {
    const [counts, myLikes] = await Promise.all([
      prisma.musicOgpGroupLike.groupBy({
        by: ['groupId'],
        where: { groupId: { in: ids } },
        _count: { id: true },
      }),
      effectiveUserId
        ? prisma.musicOgpGroupLike.findMany({
    ...
    console.error('GET /api/music/ogp-groups/likes', e);
