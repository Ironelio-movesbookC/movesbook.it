...
/** DELETE /api/music/ogp-groups/[id] — creator or admin. */
export async function DELETE(
  ...
  try {
    const existing = await prisma.musicOgpGroup.findUnique({ where: { id } });
    if (!existing) {
    ...
    if (auth.isAdmin) {
      await prisma.musicOgpGroup.delete({ where: { id } });
    } else {
      await prisma.musicOgpGroup.update({
        where: { id },
    ...
    console.error('DELETE /api/music/ogp-groups/[id]', e);
    return NextResponse.json({ error: 'Failed to delete group' }, { status: 500 });
  }
}

/** PATCH /api/music/ogp-groups/[id] — topic, name, description, visibility (creator or admin). */
export async function PATCH(
  ...
  try {
    ...
    if (data.name && data.name !== existing.name) {
      const clash = await prisma.musicOgpGroup.findUnique({
    ...
    await prisma.musicOgpGroup.update({ where: { id }, data });
    return NextResponse.json({ ok: true });
  } catch (e) {
    console.error('PATCH /api/music/ogp-groups/[id]', e);
