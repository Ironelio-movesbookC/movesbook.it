...
export async function POST(request: NextRequest) {
  try {
    ...
    if (sectionsToDelete.length > 0) {
      await prisma.nutritionSection.deleteMany({
        where: {
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      if (section.id && existingIds.has(section.id)) {
        await prisma.nutritionSection.update({
          where: { id: section.id },
          data: sectionData
        });
      ...
      // Otherwise, create a new section
        ...
        await prisma.nutritionSection.create({
          data: sectionData
        });
    ...
    // Fetch updated list
    const updatedSections = await prisma.nutritionSection.findMany({
      where: { userId: decoded.userId },
