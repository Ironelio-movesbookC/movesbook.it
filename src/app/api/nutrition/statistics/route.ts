...
// GET /api/nutrition/statistics - Get workout completion statistics
export async function GET(request: NextRequest) {
  try {
    ...
    if (startDate || endDate) {
      whereClause.nutritionDay.date = {};
      if (startDate) whereClause.nutritionDay.date.gte = new Date(startDate);
      if (endDate) whereClause.nutritionDay.date.lte = new Date(endDate);
    }

    // Fetch all workout sessions
    const sessions = await prisma.nutritionMeal.findMany({
      where: whereClause,
      include: {
        nutritionDay: {
          include: {
    ...
    sessions.forEach(session => {
      ...
      session.nutritionFoods.forEach(mf => {
        ...
        // Sum distance from nutrition_components
        mf.nutritionComponents.forEach(lap => {
          if (lap.distance) {
      ...
      // Count by week
      const weekKey = `W${session.nutritionDay.weekNumber}`;
