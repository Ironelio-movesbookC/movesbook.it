...
import { mapNutritionPlanForClient } from '@/lib/nutritionPlanClient';
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (actualPlanType === 'YEARLY_PLAN') {
      planStorageZone = 'B';
    } else if (actualPlanType === 'MEALS_DONE') {
      planStorageZone = 'C';
    ...
    if (shouldRecreate && plan) {
      ...
      const deletedDaysInRange = await prisma.nutritionDay.deleteMany({
        where: {
          userId: dbUserId,
      ...
      const deletedWeeks = await prisma.nutritionWeek.deleteMany({
        where: {
    ...
    if (!plan) {
      ...
      const newPlan = await prisma.nutritionPlan.create({
        data: {
          ...
          name: actualPlanType === 'TEMPLATE_WEEKS' ? `Weekly Plan ${section}` :
                ...
                actualPlanType === 'MEALS_DONE' ? 'Workouts Done' : 'Archive',
          type: actualPlanType as any,
      ...
      // Determine storage zone for loading days (must match plan section A/B/C)
      let loadStorageZone: 'A' | 'B' | 'C' | 'D' = planStorageZone;
      if (actualPlanType === 'YEARLY_PLAN') {
        loadStorageZone = 'B';
      } else if (actualPlanType === 'MEALS_DONE') {
        loadStorageZone = 'C';
    ...
    return NextResponse.json({ plan: mapNutritionPlanForClient(plan) });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ plan: mapNutritionPlanForClient(fullPlan) });
  } catch (error) {
