...
const PROFESSION_TABLE_CANDIDATES = ['club_setting_professions', 'club_setting_profession'];
const PROFESSION_TABLE = 'club_setting_professions';

const PROFESSION_COLUMN_DEFINITIONS: Record<string, string> = {
...
async function ensureProfessionsTable(): Promise<string> {
  const existing = await findExistingTable(PROFESSION_TABLE_CANDIDATES);
  const tableName = existing ?? PROFESSION_TABLE;

  if (!existing) {
    const columnSql = Object.entries(PROFESSION_COLUMN_DEFINITIONS)
    ...
    await prisma.$executeRawUnsafe(`
      CREATE TABLE IF NOT EXISTS \`${tableName}\` (
        ...
        ${columnSql},
        INDEX idx_club_setting_professions_user_club (user_id, club_id),
        INDEX idx_club_setting_professions_profession (profession)
      )
    `);
  }

  const columns = await getTableColumns(tableName);
  for (const [column, definition] of Object.entries(PROFESSION_COLUMN_DEFINITIONS)) {
...
function parsePayload(body: unknown): ProfessionPayload {
...
function validatePayload(payload: ProfessionPayload): Record<string, string> {
  const fieldErrors: Record<string, string> = {};

  if (!payload.name) {
    fieldErrors.name = 'Please enter profession.';
  } else if (payload.name.length > 250) {
    fieldErrors.name = 'Profession must be 250 characters or fewer.';
  }
...
async function assertOwnedProfession(
  tableName: string,
  context: AuthorizedContext,
  id: string
): Promise<boolean> {
...
function normalizeProfession(row: ProfessionRow) {
  return {
    id: String(row.id),
    name: text(row.profession),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const tableName = await ensureProfessionsTable();
    const sort = request.nextUrl.searchParams.get('sort');
    const orderBy = sort === 'alpha' ? 'profession ASC, id ASC' : 'id DESC';
    const userPlaceholders = context.userIds.map(() => '?').join(',');
    const rows = await prisma.$queryRawUnsafe<ProfessionRow[]>(
      `SELECT id, user_id, club_id, profession, created, modified
       FROM \`${tableName}\`
       WHERE user_id IN (${userPlaceholders})
       ORDER BY ${orderBy}`,
      ...context.userIds
    ...
    return NextResponse.json({
      club: context.club,
      items: rows.map(normalizeProfession)
    });
  } catch (error) {
    console.error('GET /api/club/settings/tables/professions:', error);
  ...
  try {
    ...
    if (!(await ensureNoDuplicate(tableName, context, payload.name))) {
      return NextResponse.json({
        ...
        fieldErrors: { name: 'This profession already exists.' }
      }, { status: 409 });
    }

    const storageUserId = context.legacyUserId ?? context.userId;
    await prisma.$executeRawUnsafe(
      `INSERT INTO \`${tableName}\` (user_id, club_id, profession)
       VALUES (?, ?, ?)`,
      storageUserId,
    ...
    console.error('POST /api/club/settings/tables/professions:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!(await assertOwnedProfession(tableName, context, id))) {
    ...
    console.error('PUT /api/club/settings/tables/professions:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error('DELETE /api/club/settings/tables/professions:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
