...
export async function GET(request: NextRequest) {
  ...
  try {
    const list = await prisma.newsTypedArticle.findMany({
      where: { userId },
    ...
    console.error('GET /api/news/typed', e);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const created = await prisma.newsTypedArticle.create({
    ...
    console.error('POST /api/news/typed', e);
