...
/** DELETE /api/exercises/ogp-groups/[id] — creator or admin. */
export async function DELETE(
  ...
  try {
    const existing = await prisma.exerciseOgpGroup.findUnique({ where: { id } });
    if (!existing) {
    ...
    if (auth.isAdmin) {
      await prisma.exerciseOgpGroup.delete({ where: { id } });
    } else {
      await prisma.exerciseOgpGroup.update({
        where: { id },
    ...
    console.error('DELETE /api/exercises/ogp-groups/[id]', e);
    return NextResponse.json({ error: 'Failed to delete group' }, { status: 500 });
  }
}

/** PATCH /api/exercises/ogp-groups/[id] — topic, name, description, visibility (creator or admin). */
export async function PATCH(
  ...
  try {
    ...
    if (data.name && data.name !== existing.name) {
      const clash = await prisma.exerciseOgpGroup.findUnique({
        where: {
    ...
    await prisma.exerciseOgpGroup.update({ where: { id }, data });
    return NextResponse.json({ ok: true });
  } catch (e) {
    console.error('PATCH /api/exercises/ogp-groups/[id]', e);
