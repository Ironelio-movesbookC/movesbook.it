...
/**
 * GET /api/exercises/ogp/likes?ids=id1,id2,id3
...
export async function GET(request: NextRequest) {
  ...
  try {
    const [counts, myLikes] = await Promise.all([
      prisma.exerciseOgpArticleLike.groupBy({
        by: ['exerciseOgpArticleId'],
        where: { exerciseOgpArticleId: { in: ids } },
        _count: { id: true },
      }),
      effectiveUserId
        ? prisma.exerciseOgpArticleLike.findMany({
            where: { exerciseOgpArticleId: { in: ids }, userId: effectiveUserId },
            select: { exerciseOgpArticleId: true },
          })
        : [],
    ]);

    const countMap = new Map(counts.map((c) => [c.exerciseOgpArticleId, c._count.id]));
    const mySet = new Set(myLikes.map((l) => l.exerciseOgpArticleId));
    ...
    console.error('GET /api/exercises/ogp/likes', e);
