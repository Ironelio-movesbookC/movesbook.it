/**
 * TypeScript Type Definitions for NutritionMeal System
 * Replaces 'any' types with proper type safety
 */

import type { Sport, StatusColor, NutritionMealStatus, RestType } from '@/config/nutrition.constants';

// Re-export types from constants for convenience
export type { NutritionMealStatus, StatusColor, RestType, Sport } from '@/config/nutrition.constants';
...
export interface NutritionPlan extends BaseEntity {
  userId: string;
  type: 'CURRENT_WEEKS' | 'YEARLY_PLAN' | 'MEALS_DONE' | 'ARCHIVE';
...
export interface NutritionMeal extends BaseEntity {
  dayId: string;
...
/** Legacy alias used across planner UI copied from workouts. */
export type Workout = NutritionMeal;

export interface NutritionFood extends BaseEntity {
...
export interface NutritionComponent extends BaseEntity {
  nutritionFoodId: string;
  index: number; // 1..n
  mfCode?: string | null; // Parent nutritionFood code
...
export interface UserSettings extends BaseEntity {
  userId: string;
  nutritionPreferences?: WorkoutPreferences;
...
export interface ColorSettings {
  ...
  nutritionFoodBackground?: string;
  nutritionComponentBackground?: string;
  dayBorder?: string;
  workoutBorder?: string;
  nutritionFoodBorder?: string;
  nutritionComponentBorder?: string;
  dayHover?: string;
  workoutHover?: string;
  nutritionFoodHover?: string;
  nutritionComponentHover?: string;
...
export interface CreateNutritionFoodData {
  sport: Sport;
  sectionId?: string;
  description?: string;
  nutritionComponents: CreateNutritionComponentData[];
}

export interface CreateNutritionComponentData {
  distance?: number;
  speedCode?: string;
  style?: string;
  pace?: string;
...
export interface DragData {
  type: 'workout' | 'nutritionFood';
  item: NutritionMeal | NutritionFood;
  sourceDay?: NutritionDay;
  sourceWorkout?: Workout;
}

export interface DropTarget {
  type: 'day' | 'workout' | 'nutritionFood';
  target: NutritionDay | NutritionMeal | NutritionFood;
}
...
export const isNutritionDay = (item: any): item is NutritionDay => {
  return item && typeof item === 'object' && 'date' in item && 'weekday' in item;
};

export const isWorkout = (item: any): item is NutritionMeal => {
  return item && typeof item === 'object' && 'sessionNumber' in item && 'nutritionFoods' in item;
};

export const isNutritionFood = (item: any): item is NutritionFood => {
  return item && typeof item === 'object' && 'code' in item && 'sport' in item && 'nutritionComponents' in item;
};

export const isNutritionComponent = (item: any): item is NutritionComponent => {
  return item && typeof item === 'object' && 'index' in item && 'nutritionFoodId' in item;
};
