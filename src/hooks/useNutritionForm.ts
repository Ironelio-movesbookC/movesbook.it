 ...
 * Extracted from AddMealModal.tsx
 */
export function useNutritionForm({
  ...
  const getSportsFromNutritionFoods = useCallback(() => {
    ...
    // If editing, also check the current workout's nutrition_foods
    if ((mode === 'edit' || mode === 'view') && existingWorkout && existingWorkout.nutritionFoods) {
      existingWorkout.nutritionFoods.forEach((mf: any) => {
        if (mf.sport) {
          sports.add(mf.sport);
        }
      });
  ...
  /**
   * Get nutritionFood sports (memoized)
   */
  const nutritionFoodSports = getSportsFromNutritionFoods();
  ...
  /**
   * Check if a sport at a specific index is from nutritionFoods (read-only)
   */
  const isSportFromNutritionFood = (index: number) => {
    const sport = formData.sports[index];
    return sport !== null && nutritionFoodSports.includes(sport);
  };
  ...
  const canManuallySelectSport = (index: number) => {
    // If this slot already has a nutritionFood sport, it's read-only
    if (isSportFromNutritionFood(index)) return false;
   ...
   * NOTE: Sports are now auto-loaded from nutrition_foods, so validation is not required
   */
  const validateSports = (sports: (string | null)[]) => {
    // Always valid - sports will be loaded from nutrition_foods
    return { valid: true, message: '' };
  };
  ...
  useEffect(() => {
    if (isOpen) {
      if ((mode === 'edit' || mode === 'view') && existingWorkout) {
        ...
        // In add mode, pre-populate with nutritionFood sports from the day
        const nutritionFoodSportsFromDay = getSportsFromNutritionFoods();
        const initialSports: (string | null)[] = [null, null, null, null];
        
        // Fill first N slots with nutritionFood sports (these will be read-only)
        nutritionFoodSportsFromDay.forEach((sport, index) => {
          if (index < 4) {
  ...
  }, [isOpen, mode, existingWorkout, existingWorkouts, generateDefaultCode, generateDefaultName, getSportsFromNutritionFoods]);
